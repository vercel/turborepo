...
import { PackageManager, PackageJson } from "../../src/types";
    ...
    test.each<[PackageManager, boolean]>([
      ...
      async (from, shouldThrow) => {
        ...
        expect(project.paths.root).toMatch(
          new RegExp(`^.*npm\/${directoryName}$`)
        );
        expect(project.paths.lockfile).toMatch(
          new RegExp(`^.*npm\/${directoryName}\/package-lock.json$`)
        );
        expect(project.paths.packageJson).toMatch(
          new RegExp(`^.*npm\/${directoryName}\/package.json$`)
        );
        // workspaceData
