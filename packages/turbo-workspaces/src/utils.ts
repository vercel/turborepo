...
import type { Project, Workspace, WorkspaceInfo } from "./types";
 ...
 *
 * NOTE: It's possible a project could work with bun workspaces, but just not in the way it is currently defined. We will
 * not change existing globs to make a project work with bun, we will only convert projects that are already compatible.
 *
 * This function matches the behavior of bun's glob validation: https://github.com/oven-sh/bun/blob/main/src/install/lockfile.zig#L2889
 */
function isCompatibleWithBunWorkspace({
  project,
}: {
  project: Project;
}): boolean {
  const validator = (glob: string) => {
    if (glob.includes("*")) {
      ...
      const withoutLastPathSegment = glob
        .split(path.sep)
        .slice(0, -1)
        .join(path.sep);
