...
export const date = "2022/09/19";
export const tag = "web development";
export const ogImage = "/images/blog/turbo-1-5-0/x-card.png";
...
We're delighted to announce that [`turbo prune`](/repo/docs/reference/command-line-reference/prune) now supports in pnpm, yarn, and yarn 2+.
...
`globalEnv` and `env` allow you to specify a list of environment variables _without_ `$` prefixes. This makes the configuration file significantly easier to read. Read more [in our updated docs](/repo/docs/core-concepts/caching/environment-variable-inputs).
