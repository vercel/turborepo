...
func (c *PruneCommand) Run(args []string) int {
	...
	lockfileWriter := bufio.NewWriter(tmpGeneratedLockfile)
	if err != nil {
	...
	generateLockfile, err := os.Open(filepath.Join(filepath.Join(pruneOptions.cwd, "out", "yarn.lock")))
	if err != nil {
		c.logError(c.Config.Logger, "", fmt.Errorf("failed to massage lockfile: %w", err))
		return 1
	}

	scan := bufio.NewScanner(generateLockfile)
	...
	lockfileWriter.Flush() // make sure to flush the log write before we start saving it.
	tmpGeneratedLockfile.Close()
	generateLockfile.Close()
