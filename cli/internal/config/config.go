...
func New(logger *logger.Logger, version string) (*Config, error) {
	...
	apiClient := client.NewClient(partialConfig.ApiUrl, hlogger, version, partialConfig.TeamId, partialConfig.TeamSlug, uint64(maxRemoteFailCount))
	...
	cfg.ApiClient.SetToken(partialConfig.Token)

	return cfg, nil
}

func (c *Config) SetLoggerName() func(cmd *cobra.Command, args []string) {
	return func(cmd *cobra.Command, args []string) {
		c.logOptions.Name = cmd.Name()
	}
}

func (c *Config) SetLogOptions(logger *logger.Logger) error {
	// Determine our log level if we have any. First override we check if env var
	level := hclog.NoLevel
	if v := os.Getenv(EnvLogLevel); v != "" {
		level = hclog.LevelFromString(v)
		if level == hclog.NoLevel {
			return logger.Errorf("%s value %q is not a valid log level", EnvLogLevel, v)
		}
	}
	...
	c.logOptions.Level = level
	c.logOptions.Color = color
	c.logOptions.Output = output
