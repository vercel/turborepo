...
export class Monorepo {
  ...
  init(npmClient: NPMClient, turboConfig = {}, subdir?: string) {
    // we need to *install* pnpm6 or 7, we expect 7 to be on base system
    // depends on how pnpm was installed on the base system :)
...
`,
  ...
  run(command, args?: readonly string[], options?: execa.SyncOptions<string>) {
    switch (this.npmClient) {
      ...
      case "pnpm6":
        return execa.sync("npx pnpm@6.26.1", [command, ...(args || [])], {
          cwd: this.root,
          shell: true,
          ...options,
        });
