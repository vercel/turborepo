...
| [strict](#strict) | Only allow environment variables listed in [the `env` key](/repo/docs/reference/configuration#env) for the task to be available |
...
| Git commits | Using Git specifiers, specify packages with changes according to source control | `turbo run build --filter=HEAD^1`  |
