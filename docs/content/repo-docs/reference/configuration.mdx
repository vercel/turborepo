...
<Callout type="info">
Changing your root `turbo.json` file will invalidate the cache for all tasks because it's considered in [the global hash](/TODO-LINK). If you'd like the flexibility to be able to change your pipelines without impacting the global hash, you can use [Package Configurations](/TODO-LINK).
</Callout>
...
Extend from the root `turbo.json` to create specific configuration for a package.

- Today, the only valid value for `extends` is `["//"]` to indicate inheriting configuration from the root `turbo.json`.
