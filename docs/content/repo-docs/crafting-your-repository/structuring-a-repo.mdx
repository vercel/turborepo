...
`turbo` is built on top of [Workspaces](https://vercel.com/docs/vercel-platform/glossary#workspace), a feature of package managers in the JavaScript ecosystem that allows you to group multiple packages in one repository. Following these conventions is important because it allows you to:
...
Below are the structural elements of `create-turbo` that make it a valid workspace with Turborepo on top.
...
### Specifying packages in your monorepo
...
<Callout type="error">
Turborepo does not support nested workspaces like `apps/**` or `packages/**`. Using a structure that would put a package at `apps/a` and another at `apps/a/b` will result in an error.
</Callout>
</Step>
...
A lockfile is key to reproducible behavior for both your package manager and `turbo`. Without one, there is no information about if a dependency has changed since the last time you ran `turbo` or if it was installed in a different version.
...
It's often best to start thinking about designing a package as its own unit within the workspace. At a high-level, each package is almost like its own project, with its own `package.json`, tooling configuration, and source code. There are limits to this idea—but its a good mental model to _start_ from.
...
The `scripts` field is used to define scripts that can be run from the command line in the package's context. We'll talk more about these scripts when it comes time to discuss [Running Tasks](/repo/docs/crafting-your-repository/running-tasks).
...
- **Avoiding barrel files**: Barrel files are files that re-export other files in the same package, creating one entrypoint for the entire package. While they might appear convenient, they're extremely difficult for compilers and bundlers to understand and can quickly lead to performance problems.
