...
- If you're starting from scratch, we recommend [creating a new repository using `create-turbo`](/repo/docs/getting-started/quickstart) and editing the `turbo.json` file.
...
Each key in the `pipeline` object is a task that can be executed by `turbo run`. Turborepo will search your packages for **scripts in `package.json` that have the same name as the task**.
...
If you run `turbo run build` at this point, Turborepo will run the `build` task in all packages in parallel - and you're likely to see errors. You're missing a few important pieces to make this work how you'd expect.
