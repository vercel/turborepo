...
To define a task, use [the `pipeline` object](/repo/docs/reference/configuration#pipeline) in `turbo.json`. For example, a basic task named `build` might look like this:

```json title="./turbo.json"
{
  "pipeline": {
    "build": {} // [!code highlight] // Likely to cause errors without more configuration!
  }
}
```
...
**But be careful.** At this point, we haven't marked the build outputs for caching. To do so, jump to the [Specifying outputs](#specifying-options) section.
