...
Internal Packages are the building blocks of your workspace, giving you a powerful way to share code and functionality across your repo. Additionally, where you choose to install your Internal Packages creates a dependency graph for Turborepo to understand your workspace.

## Create your first Internal Package

Let's create your first Internal Package for sharing math utilites in your repo using the guidance in the [Anatomy of a package](/repo/docs/crafting-your-repository/structuring-a-repo#anatomy-of-a-package) section. In the steps below, we're going to assume you've run `npx create-turbo@latest` or are using a similarly structured repository.
...
<Callout type="info">
The `@repo/typescript-config` devDependency is an Internal Package in `create-turbo` that we're using to manage our TypeScript configuration.
</Callout>
...
- `**devDependencies**`: Install `typescript` as a devDependency so we can compile it. In a real-world package, you will likely have more `devDependencies` and `dependencies` - but we can keep it simple for now.
