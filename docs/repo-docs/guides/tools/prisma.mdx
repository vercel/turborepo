...
Set up the Prisma client for the package.
...
Next, export the `@prisma/client` and types so it can used in your applications. Let's add an `index.ts` file to `packages/database`:
