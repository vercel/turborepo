...
- When `--continue` is `none` and an error occurs, `turbo` will cancel all tasks.
- When `--continue` is `independent-tasks-only` and an error occurs, `turbo` will cancel all dependent tasks but continue running other independent tasks.
- When `--continue` is `all` and an error occurs, `turbo` will continue running pending tasks (including dependents).
- When `--continue` is specified without a value, it will default to `all`.

In all cases, `turbo` will exit with the highest exit code value encountered during executio n.
