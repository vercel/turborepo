...
Adds a tag to a package. Does not work on the root `turbo.json`, only in package configuration. Currently only used for [`turbo boundaries`](/repo/docs/reference/boundaries).
