...
Package configuration is also used to declare tags for boundaries. To do so, add a `tags` field to your `turbo.json`:
...
From there, you can define rules for which dependencies or dependents a tag can have. Check out the [Boundaries documentation](/repo/docs/reference/boundaries) for more details
