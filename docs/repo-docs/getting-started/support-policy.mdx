...
When an API reaches stability, it is considered ready for use in production. APIs marked as stable will only change in a backward-compatible way unless there is a major release.

In the event a stable API is going to see breaking changes in an upcoming major release, warnings and documentation will be provided to help with migrations and next steps.

In the event a stable API is going to be removed in an upcoming major release, it will be marked as [Deprecated](#deprecated) to provide early notice of the removal.

### Beta

Beta means the API requires more confidence around reliability before a stable release. In the Beta phase, we are confident about the overall design of the feature and expect to be able to promote the feature to stable with relatively few changes. In rare cases, an API may require a breaking change and will not be covered by semver.
...
- [`--output=json` for the `affected` flag](/repo/docs/reference/run#--outputjson)

### Experimental

Experimental APIs lack essential capabilities required to be ready for stability. The design of the feature is considered to be under active development and can change at any time. We do not recommend using experimental APIs where reliability is essential. We encourage you to test experimental APIs in side projects and proof-of-concepts to aid in the API's development.
