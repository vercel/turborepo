...
Frameworks like Vite are using a technique where in development mode they _don’t_ bundle application source code. Instead, they serve it over native ESM - letting the browser take over part of the job of bundling the app. This results in a big speed boost at build time, since there is less bundling being done.
...
Turbopack’s development mode builds a minimal graph of your app’s imports and exports based on received requests, and only bundles the minimal code necessary. Learn more in the [core concepts docs](core-concepts).
