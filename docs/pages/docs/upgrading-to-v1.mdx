...
For good hygeine, ensure you logout of `turbo` to remove old credentials.
...
## Install the latest release of `turbo`
...
As mentioned, Turborepo now provides zero-config remote caching through [Vercel](https://vercel.com). Remote Caching is free for all Vercel plans during this transition period. Each Vercel account has a shared Remote Cache. This cache is shared across all enviroments (Development, Preview, and Production).
...
- Replace your turbrepo.com personal access token with a new [Vercel personal access token](https://@todo) and update `TURBO_TOKEN` environment variable or equivalent usage of the `--token` CLI flag
...
If you are having difficulty upgrading please file an issue on GitHub. If you are having difficulty with your remote caching on Vercel, please reach out in [Disord](https://turborepo.org/discord)
