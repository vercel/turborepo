...
- Replace your turborepo.com personal access token with a new [Vercel personal access token](https://@todo) and update `TURBO_TOKEN` environment variable or equivalent usage of the `--token` CLI flag.
