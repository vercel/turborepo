...
When you move, delete, or rename your workspaces, you must ensure that all folders linked within your `package.json` should match. Any time you change your workspace configuration, provide all the dependencies of the workspace that are updated. Re-run your package manager's `install` command to check your configuration. If there are problems after that, you may have to delete each `node_modules` folder and re-run an install.
