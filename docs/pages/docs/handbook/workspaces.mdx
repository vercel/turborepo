...
1. [Publish packages](/docs/handbook/publising-packages/basics): it'll be published on npm under the `name` you specify
...
Just like a normal package, we'd need to run `install` from root afterwards. Once installed, we can use the workspace as if it were any other package from `node_modules`. See our [section on sharing code](/handbook/sharing-code/basics) for more information.
