...
Monorepos let you share code across applications without friction. To do that, you'll be using **workspace packages** to share code between your apps.
...
2. A workspace containing code that can be shared between applications, usually inside `/packages`.

This dual meaning can be very confusing to folks new to the monorepo scene. You're likely very familiar with `1`, but not so familiar with `2`.

The fact is that they're very similar. A package is just a piece of shared code. Except that `1` lives in your `node_modules`, and `2` lives in a workspace - likely in your `/packages` folder.
...
[**Internal** packages](/docs/handbook/sharing-code/internal-packages) are intended to only be used inside the monorepo where they're housed. They are relatively simple to set up, and if your project is closed source they will be the most useful to you.

[**External** packages](/docs/handbook/sharing-code/external-packages) are bundled and sent to a package registry. This is useful for design systems, shared utility libraries or any open source work. However, they introduce more complexity around bundling, versioning and publishing.
