...
Because of Turborepo's [caching](/docs/core-concepts/caching), this also means that only repositories that have changed files will be tested - resulting in a lot of time saved.

## Running tests in watch mode

In Turborepo, 'watch' tasks behave differently to normal tasks. When you run your test suite normally, it completes and outputs to `stdout`. This means you can [cache it](/docs/core-concepts/caching) with Turborepo.

But when you run your tests in 'watch' mode, the process never exits. This makes a watch task more like a [development task](/docs/handbook/dev).
