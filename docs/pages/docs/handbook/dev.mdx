...
While developing, you'll often need to use environment variables. These let you customize the behaviour of your program - for instance, pointing to a different `DATABASE_URL` in dev and production.
...
<Tabs items={['npm', 'yarn', 'pnpm']} storageKey="selected-pkg-manager">
  ...
  <Tab>
    ```bash
    # Installs dotenv-cli in the root workspace
    yarn add dotenv-cli -W
    ```
  </Tab>
  <Tab>
    ```bash
    # Installs dotenv-cli in the root workspace
    pnpm install dotenv-cli -w
    ```
  </Tab>
</Tabs>
...
And your environment variables will be populated! In node, these are available on `process.env.DATABASE_URL`.
