...
However, unlike Turborepo, Lerna has no ability to [cache prior work](../caching) or [intelligently schedule tasks (i.e. create pipelines)](../pipelines).

### Task Scheduling

Turborepos's task scheduler is more powerful than Lerna's (which can only schedule one task at a time) thanks to [Turborepo pipelines](../pipelines). Thanks to pipelines, Turborepo reduces idle CPUs, saves compute resource, and collapse waterfalls—which ultimately results in faster overall builds.
