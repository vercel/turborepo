...
This command will generate folder called `out` by default (can be set via the `--output` option) with the following inside of it:
...
With the `--docker` flag. The `prune` command will generate folder called `out` by default (can be set via the `--output` option) with the following inside of it:
...
Default to `out`. If set, the `prune` command will generate an output folder with the specified name, relative to the working directory of the command.
