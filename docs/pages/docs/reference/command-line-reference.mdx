...
Filter syntax is based on `pnpm`'s filter syntax.
...
###### Package specifier - Glob match package names
...
You can use [`--ignore`](#--ignore) to specify changed files to be ignored in the calculation of which packages have changed.
...
For example, match everything except `a`: `--filter=!a`. Note that you may need to escape `!` as appropriate for your shell.

###### Examples

- Test everything that has changed since the previous commit: `turbo run test --filter=[HEAD^1]`
- Build the docs app and its dependencies: `turbo run build --filter=docs...`
