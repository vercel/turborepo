...
Defaults to `standard`. Set type of replay output. Use `standard` for normal output. Use `reduced` to hide cached task logs and only show what changed. Has no effect when [`--list`](#--list) is given.

```shell
turbo logs --progress=standard
turbo logs --progress=reduced
```
