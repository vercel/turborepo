...
Default standard. Set type of progress output. Use standard for normal output. Use reduced to hide cached task logs and only show what changed.

```shell
turbo run build --progress=standard
turbo run build --progress=reduced
```
