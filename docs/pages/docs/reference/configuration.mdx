...
Defaults to `origin/master`. The base branch or your git repository. Git is used by `turbo` in its [hashing algorithm](../caching#hashing) and [`--since` CLI flag](./usage#--since).
