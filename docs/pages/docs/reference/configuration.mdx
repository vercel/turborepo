...
<Callout type="info">
  When Turborepo detects a common frontend framework in a workspace, it will automatically depend on
  ...
  Turborepo already knows that the build output will change when the value of these environment variables
