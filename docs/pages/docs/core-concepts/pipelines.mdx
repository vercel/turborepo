...
A sample pipeline that defines the root task `publish` and opts the root into `test` might look like:
...
```jsonc
{
  ...
  "pipeline": {
    ...
    // This will cause the "publish" script in the root package.json
    // to be run when "turbo run publish" is run. Since the general
    // "publish" task is not defined, only the root's "publish" script
    // will be run.
    "//#publish": {
      "dependsOn": [],
      "outputs": ["dist/**/*"],
