...
This explicit configuration for `frontend#deploy` may seem like it conflicts with the `test` and `deploy` task configurations, but it does not. Since `test` and `deploy` do not have topological dependencies (e.g. `^<task>`), they can be executed any time after their own package's `build` and `test` scripts have finished!
