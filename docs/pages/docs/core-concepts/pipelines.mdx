...
The tasks in your monorepo often need to run in a certain order. For instance, `build` might need to always be run before `test`. You might be used to writing a `package.json` script like the one below:

```json
{
  "scripts": {
    // Run the build, for instance using Next
    "build": "next build",
    // Run the build BEFORE you test
    "test": "npm run build && jest"
  }
}
```

In this situation, `test` _depends on_ `build`. But what if this workspace needs to wait for _another_ workspace to be built before it can proceed?

For example, what if your `@acme/app` workspace depends the `@acme/components` workspace, which also depends on the `@acme/shared-utils` workspace... It can get confusing.

This is your pipeline - the network of dependencies in your tasks across your monorepo.

## Your pipeline probably isn't optimized

You can specify the dependencies between tasks in your monorepo by using `turbo.json`.
