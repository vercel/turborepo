...
A monorepo can contain hundreds, or thousands, of workspaces. By default, running `turbo run dev` will execute the `dev` task in **all available workspaces**.

Turborepo supports a `--filter` flag that allows you to **select the workspaces you'd like to execute your task in**.
...
You can specify multiple filters by adding multiple `--filter` to the command:
