...
<Callout type="info">
  `turbo.json` is *always* considered a global input. If you modify `turbo.json`, all caches
  are invalidated.
</Callout>
...
Luckily, you can control `turbo`'s caching behavior (also known as "hashing") based on
