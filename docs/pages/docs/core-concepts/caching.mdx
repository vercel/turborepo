...
Turborepo ships with a **task cache**. It can cache the results of any task - meaning you can save time by skipping long-running jobs.

Each task in your monorepo has an **input** and an **output**.

- A `build` task takes in **source files as input** and returns **bundled files as output**.
- A `lint` or `test` task also takes in **source files**, but outputs to **`stdout`**.

Turborepo lets you configure each task's inputs and outputs so that you **never need to do the same work twice**.
...
1. Turborepo will **take the inputs to your task** (by default all non-gitignored files), and **turn them into a hash**, like `78awdk123`.

2. It'll then **check its local cache** for a folder named `78awdk123`.

3. If it doesn't find it, it **runs the task**.

4. Once the task is over, it **saves all the outputs** (including logs) under the hash.

## Hitting the cache

Let's say that you run the task again without changing any of the source files:

![](/images/docs/cache-hit.png)

1. The **hash will be the same** - `78awdk123`, because **the inputs haven't changed**.

2. Turborepo will find the `78awdk123` folder in its cache.

3. **Instead of running the task**, Turborepo will **replay the output** - printing the saved logs to `stdout` and restoring the output files.

Restoring files and logs from the cache happens near-instantaneously. This can take your build times from many minutes down to **less than 100ms**.
