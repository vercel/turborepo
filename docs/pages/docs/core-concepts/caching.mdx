...
With the pipeline configuration above and proper output folders, if you were to run:
...
<Callout type="info">
  Pro Tip: It is more common for monorepos to use environment variables in workspaces that contain applications (such as a website), rather than shared packages (such as a UI component libary). Thus, to get higher cache hit rates, you should likely only "depend on" environment variables in package-specific tasks in your `turbo.json` configuration.
</Callout>
