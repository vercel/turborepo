...
Turborepo can cache the results and logs of any task in your codebase—meaning you never need to do the same work twice.

## Missing the cache

Each task in your codebase has **inputs** and an **outputs**.
...
1. Turborepo will **take the inputs to your task** (by default all non-gitignored files in the workspace folder), and **turn them into a hash**, like `78awdk123`.
