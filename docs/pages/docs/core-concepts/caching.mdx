...
- A `build` task might take in **source files as input** and return **bundled files as output**.
- A `lint` or `test` task might take in **source files**, and only output to **`stdout`**.

Let's say you run a `build` task with Turborepo using `turbo run build`:

![](../../../public/images/docs/cache-miss.webp)

1. Turborepo will **take the inputs to your task** (by default all non-gitignored files in the workspace folder) and **turn them into a hash**, e.g. `78awdk123`.

2. It'll then **check its cache** for a folder named `78awdk123`.

3. If it doesn't find it, it **executes the task**.

4. Once the task is over, it **saves all the outputs** (including logs) into its cache under the hash.
...
![](../../../public/images/docs/cache-hit.webp)
