...
You can control cache fingerprinting (a.k.a. hashing) based on the values of environment variables in 2 ways:

- Any environment variable that includes `TURBO` will impact the cache fingerprint of _all_ tasks and packages.
- Including environment variables in `dependsOn` list prefixed by a `$` lets you control the hash on a per-task or per-package-task basis.
...
When caching applications or packages that inline environment variables (e.g. like Next.js, Remix), you _should_ include them in the relevant `dependsOn` key based on the granularity that meets your needs.
...
- The sorted list environment variable key-value pairs that includes `TURBO` _anywhere_ in their names (e.g. `STRIPE_PUBLIC_TURBO_SECRET_KEY` but not `STRIPE_PUBLIC_KEY`)
