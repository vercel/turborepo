...
## Alter Caching Based on Environment Variables

You can control caching based on the value of environment variables by setting `env` key where applicable.

**At the root** will tell `turbo` to include the hashed values of each environment variable it all other hashes.
**In the `pipeline`** will tell `turbo` to inclued the hashed values of each environment variable on a per-task or per-package-task basis.
  ...
  "turbo": {
    ...
    "pipeline": {
      "build": {
        ...
        "outputs": ["dist/**"]
        "env": [ // values will impact hashes of all "build" tasks
          "SOME_ENV_VAR"
        ]
      },
      "web#build": { // override settings for the "build" task for the "web" package
        "dependsOn": [
          "^build"
        ],
        "outputs": [".next/**"],
        "env": [ // values will impact the hash of "build" task for only "web" package
           "STRIPE_SECRET_KEY",
...
When caching applications or packages that inline environment variables (e.g. like Next.js, Remix), you _should_ include them in the relevant `env` key based on the granularity that meets your needs.
...
- The sorted list of environment variable key-value pairs that correspond to the environment variable names listed in the top-level [`env`](../reference/configuration#env) list.
...
- The sorted list of environment variable key-value pairs that correspond to the environment variable names listed in applicable [`pipeline.<task-or-package-task>.env`](../reference/configuration#env) list.
