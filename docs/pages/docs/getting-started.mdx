...
The [`outputs`](./features/caching#configuring-cache-outputs) array allows you to override the default cache folder(s) for a task. Finally, the `dev` task has it's chaching disabled.
