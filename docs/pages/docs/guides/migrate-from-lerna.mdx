...
The above may not not look like much. But imagine that A's `build` task takes 20 minutes and now look at when B's tests run. Whoa....yeah....scheduling can save huge amounts of time when you have long build times.

### Package Publishing, Versioning, and Changelog Generation

Lerna can version and publish packages to npm registries as well as create changelogs. It's extremely good at this, especially for releasing canary pre-releases. While Turborepo may eventually tackle these features, it is not a high priority goal &mdash; we suggest using [Changesets](https://github.com/changesets/changesets) in the meantime. If Changesets isn't for you, and you want to stick with Lerna's publish flow, you can use Lerna and Turborepo together.
...
If you aren't using yarn or npm workspaces with Lerna, you should keep around your `lerna bootstrap` command if you have one. Turborepo considers Lerna a valid workspace implementation. Everything should work as expected.
