...
First thing you'll want to setup when working with workspaces is to create a folder for your workspace.
...
<Tabs items={['npm & yarn', 'pnpm']} storageKey="workspace-config-for-different-package-managers">
  <Tab>

    For `npm` and `yarn`, you need to add the folder to the `workspaces` field in the root `package.json` file
...
Last thing necessary for your workspace to be functional is a `package.json` inside the workspace folder.
This `package.json` file will house all the information for the workspace, such as the name, version, dependencies, and scripts, just like a normal `package.json` file would.
