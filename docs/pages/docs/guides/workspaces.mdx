...
First of all it is important to understand that workspaces themselves are not implemented by turborepo.
Instead they are implemented by package managers.
Since turborepo requires the use of workspaces, we will still try to explain the basics of workspaces in this guide.
The three package managers all have their own implementation of workspaces, links to their respective documentations are listed below.
