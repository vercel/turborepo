...
A common convention that we recommend is to have top-level `apps/` and `packages/` directories. The `apps` folder can contain workspaces for launchable apps, such as a Next.js or Svelte app. The `packages` folder can contain workspaces for packages that are used by either an app, or another package. This isn't a requirement, but just a suggested directory structure.
...
When you move, delete, or rename your workspaces, you will have to make sure that all folders linked within your `package.json` matches. Anytime you change the configuration of your workspace, make sure all the dependencies of the workspace are also updated. Re-run your package manager's install command to check your configuration. If there are any problems after that, you may have to delete your `node_modules` folder and run an install again.

## Managing dependencies

To manage dependencies within workspaces from the root of your monorepo, run commands that only manage the dependencies of each workspace rather than for the entire monorepo.
