...
After [installing the `turbo`](/repo/docs/getting-started/add-to-project) package (or cloning a starter), you can start using Turborepo's command line interface (CLI) `turbo` to do all kinds of awesomeness in your monorepo.
...
Controls the available variables to your tasks based upon the values specified in the `globalEnvPassthrough` and `envPassthrough` fields in `turbo.json`.

Default is to infer based upon the presence of `globalEnvPassthrough` and `envPassthrough`.

- If `globalEnvPassthrough` is configured then `strict` mode will be enabled for _all tasks_.
- If `envPassthrough` is configured then `strict` mode will be enabled for just the specified task.

Specify `--experimental-env-mode=infer` to infer strict mode _per-task_. (Default)

Specify `--experimental-env-mode=strict` to force all tasks to be in strict mode.

Specify `--experimental-env-mode=loose` to enable the entire parent environment to be available in all of your tasks.
