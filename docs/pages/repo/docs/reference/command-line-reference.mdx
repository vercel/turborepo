...
Specify `--env=infer` to infer strict mode _per-task_. (Default)

Specify `--env=strict` to force all tasks to be in strict mode.

Specify `--env=loose` to enable the entire parent environment to be available in all of your tasks.
