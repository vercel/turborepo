...
This will remove the `<package>:<task>:` prefix when running tasks (both on execute and replay from cache). This removal will also apply to "single package" repos where the prefix is just `<task>:`.
The log output when running multiple parallel tasks will be interleaved, so another method must be used to determine which task logged that line (ie enriched JSON logs).
