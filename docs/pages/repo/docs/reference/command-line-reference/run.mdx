...
Set type of task output order. Defaults to "auto" to let
turbo decide based on its own heuristics.
...
<Callout type="info">
  ...
  If this isn't desired, then the behavior can be opted out of by setting the log order to stream or grouped directly.
</Callout>
