...
<Callout type="info">
  ...
  caching](/docs/core-concepts/caching#automatic-environment-variable-inclusion).
</Callout>
...
Defaults to `true`. Whether or not to cache the task [`outputs`](#outputs). Setting `cache` to false is useful for daemon or long-running "watch" or development mode tasks you don't want to cache. Setting this option to `false` will ignore all other options except for [`dependsOn`](#dependson).
