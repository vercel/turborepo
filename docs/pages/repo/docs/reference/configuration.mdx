...
<Callout type="info">
These values will not contribute to the hash by themselves. If you want changes in these variables to cause cache misses, you will need to include them in [`env`](#env) or [`globalEnv`](#globalenv).
</Callout>
