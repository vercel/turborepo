...
<Tabs items={['Next.js', 'Vite']} storageKey="selected-framework">
  <Tab>
    ...
    <Callout>
      Next.js 13 introduces the experimental feature `transpilePackages`, which allows for the automatic transpilation and bundling of dependencies from both local packages (e.g. monorepos) and external dependencies (`node_modules`). This feature replaces the `next-transpile-modules` package. For more information, [see the Next.js Beta documentation](https://beta.nextjs.org/docs/api-reference/next.config.js#transpilepackages).
    </Callout>
