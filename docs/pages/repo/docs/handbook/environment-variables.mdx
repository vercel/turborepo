...
If your framework automatically loads environment variables from a particular file for you, you must make sure that `turbo` knows where that framework is going to load them from. This shows a baseline configuration for Next.js and Vite, who load their environment files themselves:
