...
There may be some tasks that you need to run _before_ your persistent, long-running development tasks are running. Some examples of setup steps would be:

- Pre-building a package
- Setting up Docker containers
- Checking Node or package manager versions

If you name these setup tasks `dev`, you'll have a naming collision in `turbo.json` with your persistent tasks. Instead, you can rename those setup tasks to `pre-dev` and depend on those `pre-dev` tasks for your `dev` tasks:
...
- If your tasks produce cacheable artifacts, make sure to include those in [the `outputs` key](/repo/docs/reference/configuration#outputs) of `pre-dev`.
- If your tasks _do not_ produce cacheable artifacts, you likely want to set [the `cache` key](/repo/docs/reference/configuration#cache) to `false` to make sure it always runs first.
