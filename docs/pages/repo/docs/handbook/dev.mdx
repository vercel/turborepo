...
With a global install of `turbo`, e.g. via `npm install --global turbo`, you can do
...
`turbo` will automatically pick up which directory you're running from and filter to the correct workspace.

To run the same task from any other location, use [`--filter` syntax](/repo/docs/core-concepts/monorepos/filtering).
For example:
