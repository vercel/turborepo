...
- [bun](https://bun.sh/guides/install/workspaces)
...
<Tabs items={['npm', 'yarn', 'pnpm', 'bun']} storageKey="selected-pkg-manager">
  ...
  <Tab>
    ...
    **Install a package in a workspace**
    ```bash
    bun install <package> --workspace=<workspace>
    ```

    Example:
    ```bash
    bun install react --workspace=web
    ```

    **Remove a package from a workspace**
    ```bash
    bun uninstall <package> --workspace=<workspace>
    ```

    Example:
    ```bash
    bun uninstall react --workspace=web
    ```

    **Upgrade a package in a workspace**
    ```bash
    bun update <package> --workspace=<workspace>
    ```

    Example:
    ```bash
    bun update react --workspace=web
    ```
