...
Another way would be to instead have the same file as above on the root of the monorepo, since ESLint will look up the directory tree if it doesn't find a configuration file in the current directory.
