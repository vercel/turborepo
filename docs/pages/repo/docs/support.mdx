...
Running `turbo` on unsupported is possible by compiling from source. Building from source
is documented in the [contributing guide](https://github.com/vercel/turbo/blob/main/CONTRIBUTING.md).
If you build turbo from source, you will have to run the turbo binary directly instead of using
via the Node.js package manager (via `npm install turbo`). The npm package does not contain any
specific features, so if you are able to build the binary, you should be able to use it in the
same way as you would use the npm package.
