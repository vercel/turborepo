...
darwin builds are intended for macOS on Intel and Apple Silicon. The linux
builds are tested on Ubuntu, but we expect them to work on most
[Debian-based][3] distros. Windows builds are tested on the
[`windows-latest` runner][2] in GitHub Actions.
