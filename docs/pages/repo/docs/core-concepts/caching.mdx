...
variables after `turbo` has already calculated the hash for a particular task. For example,
...
starts or when creating a build. This poses a number of problems for Turborepo:
...
To solve all of these problems, add these `.env` files to the `globalDependencies` key:
