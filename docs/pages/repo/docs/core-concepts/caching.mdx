...
- A `build` task might have **source files as inputs** and **outputs logs to `stderr` and `stdout` as well as bundled/compiled file(s)**.
...
<Callout type="info">
  ...
  workspaces. Learn more [below](/docs/core-concepts/caching#hashing).
</Callout>
...
<Callout type="idea">
  It is more common for monorepos to use environment variables in workspaces
  that contain applications (such as a website) rather than shared packages
  (such as a UI component library). Thus, to get higher cache hit rates, you
  should likely only "depend on" environment variables in package-specific tasks
  in your `turbo.json` configuration.
</Callout>
