...
Turborepo can be **adopted incrementally**. It uses the `package.json` scripts you've already written, the dependencies you've already declared, and a single `turbo.json` file. You can **use it with any package manager**, like `npm`, `yarn`, `pnpm` or `bun`. You can add it to any monorepo in just a few minutes.

## What turborepo is not

Turborepo **doesn't handle [package installation](/repo/docs/handbook/package-installation)**. Tools like `npm`, `pnpm`, `yarn` or `bun` already do that brilliantly. But they run tasks inefficiently, meaning slow CI builds.
