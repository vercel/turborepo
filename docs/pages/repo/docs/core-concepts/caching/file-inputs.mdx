...
By default, Turborepo includes the root `package.json`, any lockfile that may be present, and the root `turbo.json` file into the global hash. You do not need to specify them separately.
...
The contents of `package.json` and `turbo.json` are automatically considered for workspace hashes. This behavior is not configurable.
