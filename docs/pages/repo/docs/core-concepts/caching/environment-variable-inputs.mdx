...
Since environment variables are not captured in code repositories they're not as easily shared across machines. Ensuring that the environment setup across two different machines is consistent is one of the most-difficult tasks for Turborepo configuration.

Turborepo provides a lot of tools to make it possible to express both how, and what environment variables your application depends on.
...
- `"!*"`: excludes from the match set all environment variables.
- `"FOO*"`: matches `FOO`, `FOOD`, and `FOO_FIGHTERS`.
...
- `"!FOO*"`: excludes from the match set all environment variables that start with `FOO`.
...
By default, and when `turbo` is invoked with `--env-mode=loose`, all environment variables from the machine are made available to every single task. This is described as "loose" environment mode. This ensures the greatest compatibility while accepting some risk that a task will implicitly respond to an unknown environment variable. A user may configure a particular environment variable that results in task behavior change, but, because it was not specified inside of `turbo.json`, the created cache artifact may not be suitable for sharing.

### Strict Mode

In strict mode, when `turbo` is invoked with `--env-mode=strict`, only important system environment variables and environment variables enumerated inside of `turbo.json` will be made available to a task. There are four fields that enable specification of environment variables:
...
Subject to addition at any time, Turborepo passes in the following three environment variables to all tasks:
...
If Turborepo successfully detects your framework you do not need to manually specify certain framework-specific environment variables inside of `turbo.json`'s pipeline configuration. The supported frameworks and the environment variables that Turborepo will detect and the env wildcards that will be include into the task's `env` key:
...
The processing of the `TURBO_CI_VENDOR_ENV_KEY` values
