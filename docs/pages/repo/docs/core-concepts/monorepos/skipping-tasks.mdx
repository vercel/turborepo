...
By default, `turbo-ignore` will run the **build** task in the current working directory. Under the hood, `turbo-ignore` uses the output of `--dry=json` from your previous commit to your current commit to identify affected packages.
...
Note that this command does not _execute_ the build task. Instead, it uses a **dry run** to see if your code changes will affect your build compared to your previous commit.
...
2. Create the `dry` output for your `test` task compared to your previous commit.
