...
This mental model is useful for understands how `dependsOn` works.
...
script, so the task graph incldues those. Turborepo calls the `ui/` package a "phantom node". Since
...
In this case, _both_ `web/` and `ui/` nodes are phantom nodes. Neither implement the `build` task,
