...
With a Workspace Configurations, you can move that `compile` task into the
...
similar, but have one significant difference: when you declare a workspace-task
in the root ` turbo.json``, it _completely_ overwrites the baseline task
configuration. With a Workspace Config, the task configuration is merged
instead. Consider the example of the monorepo with multiple Next.js apps and a
Sveltekit app again. Without a workspace task, you might configure your root `turbo.json` like this:
