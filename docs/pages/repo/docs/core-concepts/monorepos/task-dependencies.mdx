...
You've written some TypeScript in your workspaces and it's time to run `tsc` to check your types.
...
Since the `topo` task doesn't exist in your scripts, Turborepo will complete the task "instantly" and then look to any workspaces that were depending on that workspace. Because of this, your tasks will execute in parallel while still understanding their relationship to other workspaces in the task graph.
