...
packages:
  - "packages/*"
  - "apps/*"
  // Note that the below is NOT supported
  - "nested-workspaces/**"
```
  </Tab>
</Tabs>
...
Each key in the [`pipeline`](/repo/docs/core-concepts/monorepos/running-tasks) object is the name of a `package.json` script that can be executed by [`turbo run`](/repo/docs/reference/command-line-reference/run). You can specify its dependencies with the [`dependsOn`](/repo/docs/reference/configuration#dependson) key inside it as well as some other options related to [caching](/repo/docs/core-concepts/caching). For more information on configuring your pipeline, see the [`Pipelines`](/repo/docs/core-concepts/monorepos/running-tasks) documentation.
...
<Callout>
  **Note: When connecting to an SSO-enabled Vercel team, you must provide your
  team's slug as an argument to `npx turbo login`.**
</Callout>
...
- [Correctly handle environment variables](/repo/docs/core-concepts/caching/environment-variable-inputs)
