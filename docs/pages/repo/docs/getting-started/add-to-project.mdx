...
Add to the file `package.json`at the root level, a workspace config, with the name of the project created before
...
5. **Try running `build` and `lint` with `turbo`:**
...
6. **Without making any changes to the code, try running `build` and `lint` again:**
