...
When we run `turbo run lint`, Turborepo looks at each `lint` script in each workspace and runs it. For more details, see our [pipelines](/repo/docs/core-concepts/pipelines) docs.
...
You'll notice that it now only runs `docs:dev`. Learn more about [filtering workspaces](/repo/docs/core-concepts/filtering) from our docs.
...
- Need to add more tasks? Learn more about using [pipelines](/repo/docs/core-concepts/pipelines)
