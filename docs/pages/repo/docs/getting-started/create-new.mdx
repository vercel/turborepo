...
Apps and packages are able to extend this configuration the same way. For instance, if we want to add a new rule specific to the `docs` app, we would do so by creating a file like this in said directory:
