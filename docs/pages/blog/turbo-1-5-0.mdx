...
- [**`turbo prune` now supports pnpm and yarn berry**](#prune-now-supported-on-pnpm-and-yarnberry): Pruning your monorepo is now supported in `pnpm` and `yarn@berry`.
...
## Prune now supported on `pnpm` and `yarn@berry`

We're delighted to announce that `turbo prune --scope=<package-name>` is now supported in `pnpm` and `yarn@berry`.

You can use `turbo prune` to create a pruned version of your monorepo - including dependencies in your lockfile.

As part of the handbook, we've also added a section on using `turbo prune` to [build docker images](/docs/handbook/deploying-with-docker).
