...
- [**Prune now supported on pnpm**](#prune-now-supported-on-pnpm): Pruning your monorepo is now supported in `pnpm` and `yarn`, with `npm` support coming soon.
...
## Prune now supported on `pnpm`

We're delighted to announce that `turbo prune --scope=<package-name>` is now supported in `pnpm`.
