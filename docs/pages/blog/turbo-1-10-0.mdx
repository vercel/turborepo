...
- [**Generators**](#generators): Use `turbo gen` to generate source code, including creating new workspaces and copying existing ones.
...
You can [create a blank workspace from scratch](/repo/docs/core-concepts/monorepos/code-generation#add-an-empty-workspace), [copy an existing workspace](/repo/docs/core-concepts/monorepos/code-generation#copy-an-existing-workspace), and even create custom generators
...
Developers and frameworks commonly use `.env` files to load environment variables into their development environments. This can cause problems when running with turbo, as these variables can be [invisible](/repo/docs/core-concepts/caching/environment-variable-inputs#invisible-environment-variables), and are not easily
included in the hash for your task. This problem could cause an unintended cache hit while working locally.

Turbo now makes this much easier with automatic support for `.env` files.

To ensure that `turbo` includes these variables in your hash, use the `dotEnv` key. For example, here is a configuration for a Next.js application:
...
Large applications can end up with many environment variables in their `turbo.json`. This can make maintaining your `turbo.json` extremely difficult to manage and maintain.
...
By default, and when turbo is invoked with --env-mode=loose , all environment variables from the machine are made available to every single turbo task.
