...
title: Turborepo 1.4
date: 2022/07/28
description: Turborepo 1.4 introduces the Turborepo Agent, has new examples, automatically hashes environment variables, and more!
tag: web development
...
<Authors authors={[ 'anthonyshewdev', 'nathanhammond', ]} />
Turborepo 1.4 lays the foundation for the future of Turborepo. We have a long list of things we want to bring you – but all of them require the existence of *one thing*.
...
But there’s a problem: Turborepo handles many file operations – and those add up! For example, it takes .008 milliseconds to `touch` a file, which is a decent approximation of the minimum work to write a file (`fopen`, `fwrite`, `fclose`). That seems fast! But, once your build contains thousands of files those .008 milliseconds start to accumulate quickly.

![Timing of an execution of touch, demonstrating it taking .008ms to create a file.](/images/blog/turbo-1-4-0/time.png)

Say hello to the Turborepo Agent.
...
<Callout>
Running on macOS and wondering why this change doesn’t seem to make any difference? That’s because [Apple has optimized their file system operations to do the work when nobody is watching](https://twitter.com/marcan42/status/1494213862970707969). Clever, Apple. We see you.
</Callout>
...
Unsupervised processes are wasteful and the majority of the code that was added was to make process handling as robust as possible.

## New examples

Over the past month we’ve invested a lot of time in [creating new examples for how to integrate Turborepo into your workflow](https://github.com/vercel/turborepo/tree/main/examples), featuring:
...
One of the easiest mistakes to make when setting up Turborepo is accidentally omitting an important environment variable resulting in reusing a cache somewhere you didn't expect. Turborepo now has your back and will detect commonly used environment variables to help you avoid this problem.
...
- as many as 325k weekly NPM downloads
- 8.1 years of compute time saved through Remote Caching on Vercel, saving 8 months per week
