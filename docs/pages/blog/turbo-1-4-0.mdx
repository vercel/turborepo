...
Turborepo 1.4 improves usage with frameworks and adds new examples:
...
- [**`eslint-plugin-turbo`**](#eslint-plugin-turbo): Enhanced feedback with a new ESLint plugin
...
To help ensure correct caching across environments, Turborepo will now automatically infer and include public environment variables when calculating cache keys for apps built with Astro, Create React App, Gatsby, Next.js, Nuxt, SvelteKit, Vite, Vue, and more. You can safely remove these framework-specific public environment variables from `turbo.json` if you have manually declared them.
...
For example, consider a monorepo with a three workspaces: a Next.js project, a Create React App project, both depending on a shared Typescript package and each with their own`build` script. Let's say that this Turborepo has a standard `turbo.json` pipeline that builds them all in order:
...
As of 1.4, when you run `turbo run build`, Turborepo will not consider any build time environment variables relevant when building the TypeScript packages. However, when building the Next.js app, Turborepo will infer that environment variables starting with `NEXT_PUBLIC_` could alter the output of the `.next` folder and should thus be included when calculating the hash. Similarly, when calculating the hash of the Create React App's `build` script, all build time environment variables starting with `REACT_APP_PUBLIC_` will be included.

This improvement in hash specificity by framework is a significant step toward optimal, safe, and correct caching.

## `eslint-plugin-turbo`

We've also created a new ESLint plugin `eslint-plugin-turbo` to provide further in-editor assistance and conformance to help ensure your Turborepo cache can be correctly shared across every environment. While our new hashing algorithm should cover most situations with most frameworks, our new plugin will provide feedback for teams using other build time inlined environment variables that are not framework-prefixed which impact build outputs (i.e. caching) and teams using their own in-house framework which we don't infer automatically.

To get started, simply extend from `eslint-config-turbo` in your root `[eslintrc](https://eslint.org/docs/latest/user-guide/configuring/configuration-files#configuration-file-formats)` file:
...
Or, if you prefer more control over the rules - use `eslint-plugin-turbo` directly by first adding it to plugins and then configuring the desired rules:
