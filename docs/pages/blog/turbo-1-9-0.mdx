...
- [**Run Summaries**](#run-tasks-with-confidence): Use the `--summarize` flag to generate a summary of your task to compare against previous runs.
...
When this flag is enabled, Turborepo will generate a summary in `.turbo/runs/` that contains all the information necessary to understand how `turbo` interpreted your `turbo.json` and task run inputs.
...
Check out the [docs](/repo/docs/reference/command-line-reference#--summarize) to learn more.
...
```bash
npx create-turbo -e https://github.com/your-community/turbo-starter
```
...
In `strict` mode, Turborepo will pass all environment variables declared in:

- `globalEnv` and `experimentalGlobalPassThroughEnv` for global configuration
...
In `strict` mode, this configuration will only expose four environment variables to your `build` task, helping you to catch missing variables earlier in the development process.

The `--experimental-env-mode` also supports `loose` and `infer`, `infer` being the default behavior of Turborepo today.
