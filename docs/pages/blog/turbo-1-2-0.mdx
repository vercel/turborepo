...
- [**New Task Filtering API**](#new-task-filtering-api): `--filter` adds more expressive and intuitive task filtering capabilities to `turbo run`
...
With `--filter` you can tell `turbo` to restrict executing commands to a subset of matched packages in your monorepo based on name or folder, even if it has changed since a git commit ref.
...
For example, if you had an app located in `./apps/web` directory with local packages used as dependencies, and a Turborepo pipeline where `test` depends on `^build` [topologically](https://turborepo.org/docs/glossary#topological-order), running:
...
We hope that this will improve visibility into what `turbo` is doing, speeding up debugging as well as make it easier to leverage `turbo` in dynamic CI/CD systems.
...
The Turborepo team at [Vercel](https://vercel.com) is hiring! We're up to five core team members already this year and are looking to hire even more. We're specifically looking for full-time [Senior Build Systems Engineers](https://vercel.com/careers/senior-build-systems-engineer-turborepo-us-4383106004).
