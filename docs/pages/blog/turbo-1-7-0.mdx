...
You can now tell Turborepo about tasks that start long-running processes with a `persistent: true` [configuration option](/repo/docs/reference/configuration#persistent). When this config is set, Turborepo will throw an error on invalid task dependency graphs. This is useful for `dev` tasks or test runners with `--watch` flags.
