...
- [**Long-running task support**](#schedule-your-long-running-tasks-with-confidence): Use `persistent: true` to mark non-terminating tasks so that `turbo` can alert you if you have dependencies on them.
- [**Better clarity for outputs**](#declare-your-outputs-for-improved-clarity): You'll always need to declare your outputs, improving transparency to what your tasks will cache.
- [**Globally installable**](#global-turbo): Install once, use everywhere.
- [**“Error only” output mode**](#error-only-output-mode-for-quieter-logs): Quiet your output logs to only show when a task fails.
...
You can now tell Turborepo about tasks that start long-running processes with a `persistent: true` config. When this config is set, Turborepo will throw an error on invalid task dependency graphs. This is useful for `dev` tasks or test runners with `--watch` flags.
...
Up to this 1.7 release, we had been recommending the use of `turbo run <task> --parallel` for persistent tasks. With `--parallel`, `turbo` would ignore your dependency graph and execute all your tasks at once.

While `--parallel` did solve many problems with possible user misconfigurations, it means that users have to tell Turborepo **_how_** to run their tasks rather than declaring **_what_** a task is.
...
To bring visibility to errors, community member [@dobsev](https://github.com/dobesv) contributed [a solution to only show errors that result from a task pipeline](https://github.com/vercel/turbo/pull/2588). While debugging a pipeline, `--output-logs=error-only` can be used to keep your signal-to-noise ratio high so you can focus on ensuring successful runs for your pipelines.
