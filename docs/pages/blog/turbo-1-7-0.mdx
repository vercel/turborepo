...
To avoid misconfigurations that could result in tasks that never run, you can now tell Turborepo about tasks that won't exist on their own (like `dev` scripts) with a `persistent: true` [configuration option](/repo/docs/reference/configuration#persistent). When this config is set on a task, Turborepo will ensure no other task can depend on this task. This is useful for `dev` tasks or test runners with `--watch` flags.
