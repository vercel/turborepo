...
- [**Better clarity for outputs**](#declare-your-outputs-for-improved-clarity): You'll now always need to declare your tasks outputs, improving transparency to what your tasks will cache.
- [**Globally installable**](#global-turbo): Install once, use everywhere. Turborepo can now be installed globally, and ran from any directory.
...
To avoid misconfigurations that could result in tasks that never run, you can now tell Turborepo about tasks that won't exist on their own (like `dev` scripts) with a `persistent: true` [configuration option](/repo/docs/reference/configuration#persistent). When this config is set for a task, Turborepo will ensure no other task can depend on this task. This is useful for `dev` tasks or test runners with `--watch` flags.
