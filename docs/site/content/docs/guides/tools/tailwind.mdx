...
<Callout type="good-to-know">
  In order for Tailwind to include class names used in the UI package, we need to register that source directory. You can read more
  [here](https://tailwindcss.com/docs/detecting-classes-in-source-files).
</Callout>
