...
For monorepos, you may want to exclude certain directories from linting. Add an `ignorePatterns` field to your `.oxlintrc.json` configuration file:
...
Using `ignorePatterns` in the configuration file is preferred over a separate `.oxlintignore` file because:

- Developers running `oxlint` directly will have the same files ignored
- Editor integrations will automatically pick up the ignore patterns
