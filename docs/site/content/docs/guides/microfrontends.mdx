...
<Callout>
  The tutorial below is using `npx create-turbo@latest` if you want to try for
  yourself.
</Callout>
...
Ensure each microfrontend has a development task in its `package.json`:

```json title="./apps/checkout/package.json"
{
  "name": "@acme/checkout",
  "scripts": {
    "dev": "next dev", // [!code highlight]
    "build": "next build"
  }
}
```

### 4. Set up your turbo.json
...
```json title="./apps/checkout/microfrontends.json"
{
  "partOf": "web"
}
```
