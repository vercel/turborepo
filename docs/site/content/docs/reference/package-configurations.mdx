...
<Callout type="good-to-know">
  By default, array fields like `outputs` and `env` completely replace the
  values from the root configuration. To append to inherited values instead of
  replacing them, use the [`$TURBO_EXTENDS$`
  microsyntax](/docs/reference/configuration#turbo_extends).
</Callout>
