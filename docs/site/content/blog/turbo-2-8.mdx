...
Turborepo 2.8 strengthens Turborepo's position as the best build system for agentic coding, with features like:
...
- [**Task descriptions**](#task-descriptions): Add context to your task definitions
...
[Git worktrees](https://git-scm.com/docs/git-worktree) are a common technique for this. They allow you to check out multiple branches simultaneously, in separate directories, all sharing the same Git history. However, in previous versions of Turborepo, this meant that a new local cache instance would get used for each worktree.
...
# Create a worktree
git branch my-branch && git worktree add ../my-worktree my-branch

## Change to the directory and run the same task
## Full cache hit from the main worktree's local cache
cd ../my-worktree && turbo run build
```
...
- **Markdown responses for agents**: By responding with Markdown responses (instead of HTML), we help to preserve your context window. No configuration or special instructions are required. Run `curl -sL -H "Accept: text-markdown" https://turborepo.dev/repo/docs` in your terminal to see an example response.
...
Descriptions are purely informational and don't affect task execution. They're useful for:
