...
<Callout type="info">
    Turbopack is available for the Next.js development server. You can try out Turbopack today by adding the `--turbopack` flag to your `next dev` command.
...
Add `--turbopack` to your `next dev` command:
