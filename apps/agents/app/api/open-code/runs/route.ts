...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!process.env.OPENCODE_SERVER_URL || !runSecret) {
    return Response.json({ error: "OpenCode workflow is not configured." }, { status: 503 });
  }

  const body: unknown = await request.json().catch(() => null);
  const example = typeof body === "object" &&
    body !== null &&
    "example" in body &&
    ...
    ? body.example
    : null;
  if (!example) return Response.json({ error: "A valid example is required." }, { status: 400 });
