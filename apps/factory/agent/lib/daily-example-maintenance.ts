...
When changes exist, leave them uncommitted and run \`factory-create-pr --branch agents/examples-${example}-fx-${sessionId} --title "chore: Update ${example} example" --body "<summary and validation results>"\`. Factory creates the verified commit, branch, and draft pull request through Eve's GitHub credentials. Never run git commit, git push, gh auth setup-git, or gh pr create. If there are no changes, finish without creating a pull request.`;
}
