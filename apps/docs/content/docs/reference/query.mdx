...
Exit with code 1 when affected packages or tasks are found, 0 when none are found. JSON output is still printed to stdout.

This is useful for simple CI gating where you only need a binary "affected or not" signal. For most use cases, we recommend parsing the JSON output directly for more flexibility.
...
| --- | --- |
| `npx turbo-ignore my-app` | `turbo query affected --packages my-app --exit-code` |
...
- **No commit message directives**: `turbo-ignore` supported `[skip ci]` and `[vercel skip]` commit message directives. Use your CI platform's native skip features instead.
- **No `TURBO_FORCE` handling**: Check this environment variable in your CI script if needed.

#### CI example

```bash title="Terminal"
# Check if a package is affected (exit 1 = affected, 0 = not affected)
turbo query affected --packages my-app --exit-code

# Or parse the JSON output for more control
result=$(turbo query affected --packages my-app)
# Use jq, Node.js, or your preferred JSON tool to inspect the output
```
