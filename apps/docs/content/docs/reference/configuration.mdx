...
Most tasks should continue using normal string `inputs`. Use deferred hashing when your task's cache key depends on files that are not yet available or are stale when `turbo` starts.
...
In this example, `check-types` waits for `codegen`, then hashes `src/generated/**` before checking cache.

Use `dependencyOutputs` when this task should hash declared outputs from a task dependency:
