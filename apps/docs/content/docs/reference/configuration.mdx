...
#### `$TURBO_JIT$` <ExperimentalBadge>Pre-release</ExperimentalBadge>

`$TURBO_JIT$` marks file inputs that should be hashed just in time, after the
task's `dependsOn` tasks have completed.

Use this when a task depends on files that are generated by another task in the
same run. Without `$TURBO_JIT$`, those files are hashed when `turbo` starts. If
the generated files do not exist yet, or if stale generated files are present,
the downstream task can be cached under the wrong file-input hash until the next
run.
...
In this example, `check-types` waits for `codegen`, then hashes
`src/generated/**` before checking the cache for `check-types`.

You can also use `$TURBO_JIT$` with `$TURBO_ROOT$` to hash generated files across
package boundaries:
...
```jsonc title="./apps/web/turbo.json"
{
  ...
  "tasks": {
    "check-types": {
      ...
      "inputs": [
        "$TURBO_DEFAULT$",
        "$TURBO_JIT$/$TURBO_ROOT$/packages/ui/src/generated/**",
      ],
    },
  },
}
...
Negations with `$TURBO_JIT$` only apply to JIT inputs:
...
Normal negations do not exclude JIT inputs. For example,
`!src/generated/**` does not exclude `$TURBO_JIT$/src/generated/**`.

<Callout type="warn" title="Pre-release">
  `$TURBO_JIT$` is a pre-release feature. The configuration surface and output
  shown by `--dry` may change before this feature is marked stable.
</Callout>

Notes:

- `$TURBO_JIT$` only applies to task `inputs`.
- `$TURBO_JIT$` is not valid in `dependsOn`, `outputs`, or global inputs.
- A task using `$TURBO_JIT$` cannot know its final cache key until just before it
  runs. Because of this, `--dry` output shows a deferred hash message for tasks
  using `$TURBO_JIT$`, with `hashReason` mentioning the just-in-time deferral.
- `--affected` and `turbo query affected` conservatively treat tasks with
  `$TURBO_JIT$` inputs as affected, since their final inputs are not known until
  their task dependencies complete.
