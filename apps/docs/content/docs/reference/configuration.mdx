...
Configures which workspace manifest ecosystems Turborepo should use when building the workspace graph.
...
- `node` - `package.json` workspaces and scripts
- `cargo` - Rust `Cargo.toml` workspaces
- `uv` - Python `pyproject.toml` workspaces managed by uv

If omitted, Turborepo behaves like a Node-first repository (`["node"]`).
...
#### `pruneIncludesGlobalFiles` <ExperimentalBadge>Pre-release</ExperimentalBadge>
...
Without this flag, the `globalDependencies` entries are preserved in the pruned `turbo.json` but the actual files they reference (e.g. a root `tsconfig.json` or `.env`) are not copied. This can cause tasks to behave differently in the pruned output because the files they depend on are missing.
...
With this configuration, running `turbo prune frontend` will include `tsconfig.json` and `.env` in the output alongside the pruned workspaces and lockfile. Both inclusion and `!` exclusion globs are supported.
...
Set an explicit command to execute for a task.
...
When provided, `command` takes precedence over inferred commands from workspace manifests (for example Node scripts, Cargo task inference, uv task inference).

This is useful when:

- You want a non-default command for a provider task
- You need a single command that works across packages
- You are incrementally migrating task commands into `turbo.json`
