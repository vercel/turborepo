...
<Callout type="info">
  Exporter failures are logged but do not cause the run to fail. If the collector
  is unavailable or misconfigured, Turborepo will continue executing tasks normally.
</Callout>
