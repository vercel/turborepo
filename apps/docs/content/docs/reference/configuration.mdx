...
Deferred hashing is a power-user feature for tasks whose cache key depends on
files that are not reliable when `turbo` starts. This commonly happens when a
...
Most tasks should continue using normal string `inputs`. Use deferred hashing
only when startup hashing would read missing or stale files.
...
In this example, `build` still hashes at startup, using the default package
inputs while excluding `dist/**`.

Use `jit` when this task should hash files after another task creates or updates
them:
...
dependency instead of arbitrary files in its own package:
