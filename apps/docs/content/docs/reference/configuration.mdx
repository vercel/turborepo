...
`inputs` accepts string globs and, as a pre-release feature, structured input
objects that make input timing explicit.
...
#### Structured inputs <ExperimentalBadge>Pre-release</ExperimentalBadge>

Structured inputs describe when files are hashed instead of encoding that
behavior in special glob strings.

<Callout type="warn" title="Pre-release">
  Structured inputs are a pre-release feature. The configuration surface and
...
| Mode | What it does |
| --- | --- |
| `startup` | Hashes files before the task graph starts executing. This is equivalent to normal string inputs. |
| `jit` | Hashes files after the task's `dependsOn` tasks complete, just before this task checks cache. |
| `dependencyOutputs` | Hashes declared `outputs` from selected dependency tasks after those tasks complete. |

Use `startup` when you want normal input hashing with structured syntax:
...
In this example, `build` hashes the default package inputs while excluding
`dist/**`.

Use `jit` when another task creates files that this task should hash:
...
In this example, `check-types` excludes generated files from startup hashing,
waits for `codegen`, then hashes `src/generated/**` before checking cache.

Use `dependencyOutputs` when this task should hash declared outputs from a task
dependency:
...
In this example, `check-types` hashes the files that `codegen` declares in
`outputs` and that also match `src/generated/**`. `dependencyOutputs` does not
create task graph edges; the selected task must already be present through
`dependsOn`.
...
- Tasks using `jit` or `dependencyOutputs` cannot know their final cache key
  until execution time, so `--dry=json` reports `hash: null` and a `hashReason`.
