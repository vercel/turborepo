...
You can override the default base and head with `--base` / `--head`, or with their respective [System Environment Variables](/docs/reference/system-environment-variables). CLI flags take precedence over the environment variables.

```bash title="Terminal"
# Override Git comparison base via flags
turbo run build --affected --base=development --head=HEAD

# Override Git comparison base via environment variables
TURBO_SCM_BASE=development turbo run build --affected

# Override Git comparison head via environment variables
...
```bash title="Terminal"
turbo run build --affected --head my-branch
```
...
If you need to ensure certain tasks always run together (e.g. a frontend and an API), use `with` in a [Package Configuration](/docs/reference/package-configurations):
