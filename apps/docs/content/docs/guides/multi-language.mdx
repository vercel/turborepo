...
description: Integrate languages with package scripts and workspace boundaries.
product: turborepo
type: guide
summary: Add tools without native workspace support by wrapping projects in package-manager workspaces.
prerequisites:
...
This guide uses Go as an example of the package-script integration, which remains available without experimental flags. Turborepo can also interpret `go.work` and `go.mod` through [experimental native Go workspace support](/docs/guides/tools/go). Without that opt-in, Turborepo sees only the package scripts, files, outputs, and package-manager dependency relationships described here.
...
In this package-script setup, Turborepo does not interpret `go.mod` or Go imports. It hashes files inside `services/api` as package inputs, then runs the declared scripts and lets Go resolve its own module graph. Enable [native Go workspace support](/docs/guides/tools/go) when Turborepo should discover those module relationships directly.
