...
- **Library crates** remain Package Graph nodes so filtering and affectedness calculations follow Rust dependency relationships. They expose verification tasks, while `build` remains a no-op because Cargo builds libraries as part of another crate's dependency closure.
...
# Build every Rust entrypoint in the workspace dependency closure
