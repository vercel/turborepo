...
- Go 1.22 or newer available as `go` on `PATH`. [`turbo setup`](/docs/reference/setup) installs the release named by the `toolchain` or `go` directive into the repository when it is missing
