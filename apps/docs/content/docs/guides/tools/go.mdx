...
<Callout type="warn">
  Earlier experimental releases wrote runnable-module binaries to `dist/`. Update
  deployment start commands, Docker `COPY` paths, and other binary consumers to
  use the module-local path, or [override the build command](#override-or-remove-defaults)
  to preserve your existing output location and name. Removing a `package.json`
  wrapper does not migrate its custom build command automatically.
</Callout>
