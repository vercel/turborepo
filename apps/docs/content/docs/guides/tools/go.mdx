...
| Module with exactly one `main` package | `turbo run`, `turbo dev` | `go run <import-path>` |
| Any module | `turbo test` | `go test ./...` |
| Any module | `turbo vet` | `go vet ./...` |
| Any module | `turbo lint` | The built-in `vet` task |
| Any module | `turbo format` | `go fmt ./...` |
| `go-workspace` | `test`, `vet`, `lint`, `format` | The same command with every member directory pattern |

Turborepo does not guess when a module has zero or multiple runnable `main` packages. Those modules do not receive `run` or `dev`.

Arguments after Turborepo's `--` are placed before package patterns for `build`, `test`, `vet`, and `format`. Arguments to `run` and `dev` follow the main import path and become program arguments.

```bash title="Terminal"
turbo run test --filter=example.com/acme/api -- -run TestHTTP
turbo run run --filter=example.com/acme/api -- --port 8080
```
...
A module with one runnable main package gets one restorable `dist/<name>` binary. Library builds default to uncached because Go's internal build artifacts have no stable output for Turborepo to restore. `run`, `dev`, and source-mutating `format` tasks are uncached. Build arguments disable automatic output caching because flags can relocate or reshape the binary.
...
The output keeps the selected module and required internal dependencies, rewrites `go.work` with deterministic explicit members, preserves `go`, `toolchain`, `godebug`, and applicable replacement directives, and copies `go.work.sum` plus each retained module directory.
...
- The workspace must be rooted at the Turborepo root. Members and local replacements outside the repository, or local replacements to non-members, are rejected.
- A root module that collides with a listed member, duplicate module paths, and the reserved `go-workspace` identity are rejected.
- Vendoring is not interpreted as a separate resolution source. Use current module/workspace sums and standard workspace module mode.
- Turborepo does not auto-detect `golangci-lint` and does not proxy Go's build cache.
- If Go reports a stale graph or checksum error, run the appropriate `go mod tidy` and `go work sync` commands, review the changes, and commit the updated files.
