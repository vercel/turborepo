...
Format tasks remain uncached because they mutate source files. Build tasks also remain uncached because uv can resolve isolated PEP 517 build backends independently of `uv.lock`; those backend identities are not yet represented in the task hash. The fallback `uv check` task remains uncached because uv resolves its bundled checker independently of the workspace lockfile.
...
- Isolated build-backend identities are not yet hashed, so built-in build tasks remain uncached by default.
