...
You can check if a specific task is affected using
...
If you only need a binary "affected or not" signal without parsing JSON, you can use the `--exit-code` flag as a shorthand. It exits with code `1` when your query reports something has been affected. It returns `0` when empty:
