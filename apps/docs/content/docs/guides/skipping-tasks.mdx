...
You can check if a specific task is affected, which is more precise because it respects the [`inputs`](/docs/reference/configuration#inputs) configuration for that task:
...
If you only need a binary "affected or not" signal without parsing JSON, you can use the `--exit-code` flag as a shorthand. It exits with code `1` when results are found, `0` when empty:
