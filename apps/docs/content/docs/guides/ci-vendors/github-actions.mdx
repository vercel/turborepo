...
After you [create a Turborepo CLI OIDC policy and add a `TURBO_TEAM` repository variable](https://vercel.com/docs/monorepos/remote-caching/external-ci-cd#openid-connect-oidc), add [`vercel/setup-turborepo-remote-cache-action`](https://github.com/vercel/setup-turborepo-remote-cache-action) before any step that runs `turbo`. It requests a GitHub OIDC token, exchanges it for a short-lived Turborepo access token, and sets `TURBO_TOKEN` and `TURBO_TEAM` for later steps:
...
<Callout type="info">
  If you create multiple OIDC policies on your team, Vercel will try to match
...
After you [create a scoped access token, store it as the `TURBO_TOKEN` secret, and add `TURBO_TEAM` as a repository variable](https://vercel.com/docs/monorepos/remote-caching/external-ci-cd#personal-access-token-pat), provide them to jobs that run `turbo`:
