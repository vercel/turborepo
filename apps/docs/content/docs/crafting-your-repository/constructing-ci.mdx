...
Use `turbo query affected` with `--exit-code` to get a binary signal:

<Tabs items={["Check a package", "Check a specific task", "Parse JSON output"]}>
...
```bash title="Terminal"
turbo query affected --tasks test --packages docs --exit-code
```

</Tab>

<Tab value="Parse JSON output">

For more control, parse the JSON output directly:

```bash title="Terminal"
turbo query affected --packages web
```

The output includes the reason each package is affected, which is useful for debugging.
...
With `--exit-code`, the command exits with code `1` if the package is affected by changes, and `0` if it is not.

Using this exit code, you can choose what the rest of your CI pipeline should do. For instance, a `1` exit code likely means that you should move forward with installing dependencies and running tasks.
