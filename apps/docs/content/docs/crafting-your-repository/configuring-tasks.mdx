...
#### Hashing generated inputs

Some tasks need to hash files that are created by another task in the same run.
For example, a type-check task might need to hash files produced by a `codegen`
task.

Structured task inputs are available as a pre-release feature for these cases.
Use [`mode: "jit"` or `mode: "dependencyOutputs"`](/docs/reference/configuration#structured-inputs)
to defer hashing until the task's dependencies have completed.
