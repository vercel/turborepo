...
![A graphical representation of tasks running sequentially, with lots of empty space where scripts are not being ran.](/images/docs/slow-tasks-light.png)

But, to get the same work done **faster** with Turborepo, you can use `turbo run lint build test`:

![A graphical representation of `turbo run lint test build`. It shows all tasks running in parallel, with much less empty space where scripts are not being ran.](/images/docs/turborepo-tasks-fast-light.png)
