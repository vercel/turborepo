...
```json title="./turbo.json"
{
  "tasks": {
    "build": {
      "outputs": [".next/**", "!.next/cache/**", "!.next/dev/**"] // [!code highlight]
    }
  }
}
```
...
Use [`mode: "jit"`](/docs/reference/configuration#just-in-time-jit) when a task
depends on files created by upstream tasks, or
[`mode: "dependencyOutputs"`](/docs/reference/configuration#output-dependent-dependencyoutputs)
when dependents should hash a dependency's declared `outputs`. Both modes wait
until the task's dependencies have completed before finalizing the cache key.
