...
<Date>Friday, March 27th, 2026</Date>
...
- [**Faster Time To First Task**](#96-faster): Up to 96% improvement, scaling with repository size and complexity
- [**`turbo query` is now stable**](#turbo-query-is-stable): Query your monorepo with GraphQL or shorthands like `turbo query affected`
- [**Easier adoption for large repos**](#package-graph-cycles): Circular package dependencies no longer block your Turborepo adoption
- [**OpenTelemetry (Experimental)**](#opentelemetry-experimental): Send task metrics to any OTLP-compatible observability backend
- [**Structured logging (Experimental)**](#structured-logging): Machine-readable JSON output with `--json` and `--log-file`
- [**Future Flags and deprecations**](#future-flags-and-deprecations): Prepare early for 3.0 using pre-existing migration paths

Additionally, we've closed 111 Issues since Turborepo 2.8, addressing 262 👍 reactions, reducing our total open Issue count by 48%.
...
Turborepo's most important role is to schedule your monorepo's tasks to be as fast as possible. Whenever you use `turbo run`, our first step is to analyze your monorepo's structure and scripts to create a graph of your tasks. The [Task Graph](/docs/core-concepts/package-and-task-graph#task-graph) describes what tasks depend on each other, so we know how to run work as soon as possible, and what work can be restored from cache.

Once we're done creating your Task Graph, we begin executing your tasks. "Time to First Task" is our name for the time between when `turbo run` begins execution to the first script in your monorepo being executed. In general, Time To First Task represents Turborepo's overhead in your monorepo, and we've made it dramatically faster in this release.
...
>
  ...
  <div className="p-4 flex flex-col gap-4 bg-[var(--ds-background-100)]">
    ...
    <p className="text-xs text-[var(--ds-gray-900)] mt-3">
      Benchmarked on Apple M4 Max, 36 GB RAM, macOS 15.7.1
    </p>
  </div>
</div>

The performance boost you see will depend on the size and complexity of your repository - but we expect **dramatic** improvement for all Turborepos. We've tested these improvements across many open and closed source Turborepos, and the largest performance improvement we saw was 96%.

For a technical deep dive on how we accomplished this, [read our post on the Vercel blog](TODO).

## `turbo query` is now stable

[`turbo query`](/docs/reference/query) has graduated from Experimental to Stable. Originally introduced in Turborepo 2.2, `turbo query` lets you run GraphQL queries against your monorepo's Package and Task Graphs.
...
We're also introducing a few shorthands so you don't have to write long GraphQL queries to answer your most common questions.
...
Circular package dependencies are a common reality in large monorepos. While they're not a great architectural decision, they happen. The example below shows how easily packages can mutually depend on each other.
...
| Flag                                                                                   | Description                                                                                                                                                                                                     |
| -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`globalConfiguration`](/docs/reference/configuration#globalconfiguration)             | Moves `globalDependencies`, `globalEnv`, and `globalPassThroughEnv` under a [`global` key](/docs/reference/configuration#global). `globalDependencies` becomes `global.inputs` with per-task hashing semantics. |
...
| -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| [`turbo-ignore`](/docs/reference/turbo-ignore)                                                                                   | [`turbo query affected`](/docs/reference/query)                                                                           |
| [`turbo scan`](/docs/reference/scan)                                                                                             | Removed, obsolete functionality                                                                                           |
...
<Accordions>
  ...
  <Accordion title="Documentation (34)">
...
We also thank everyone who contributed to this release of Turborepo: @04cb, @alexmkio, @bestickley, @bkonkle, @ddmoney420, @EmojiPati, @GabrielBB, @Goldyvaiiii, @haydenbleasel, @i5d6, @Jaredw2289-svg, @kyedavey, @lilianakatrina684-a11y, @marko-hologram, @mehulkar, @molebox, @MrNaif2018, @Netail, @odaysec, @PeterPCW, @prudentbird, @ramanverse, @Rohan5commit, @serjooo, @sleitor, @styfle, and @veeceey.
