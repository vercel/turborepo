...
- [**`$TURBO_JIT$` for generated file inputs**](#turbo_jit-for-generated-file-inputs): Defer a task's hashing until the moment it starts
...
It's common for a repository to have a shutdown or cleanup step to tear down a development environment. Examples include cleanly shutting down a server, terminating a Docker Compose script, or closing a background process.

In previous versions of Turborepo, tasks that wanted to gracefully shut down were not respected. Turborepo would immediately exit, abruptly killing all of your tasks, and possibly leaving your development environment in an undesirable state.
...
This behavior is zero-configuration, and if your repository does not have tasks that gracefully shutdown, you will see no observable change. [Visit the documentation](/docs/reference/run#graceful-shutdown) to learn more

## Local cache eviction

Turborepo saves its local cache into `.turbo/cache`, leaving behind cache artifacts every time you run tasks. While this makes your repository faster while working locally, there's one problem: The local cache can keep growing forever - and at accelerated rates.
...
[Visit the documentation](/docs/reference/configuration#cachemaxage) to learn more. We expect to enable this configuration by default in Turborepo 3.0.

## Combine `--affected` and `--filter`

Previously, you couldn't use `--affected` and `--filter` at the esame time. Now, you can.
...
These flags will combine with intersection semantics, only running the tasks that match **both** constraints.

[Visit the documentation](/docs/reference/run#--affected) to learn more.

## `$TURBO_JIT$` for generated file inputs

Turborepo creates the hashes for all of your tasks immediately, right when `turbo` starts. This lets Turborepo get work done as fast as possible, since it can instantly know whether your work is a cache hit or miss.

However, this becomes problematic in certain situations. For example, if you have a task that does code generation, it might change the files for a package's file inputs **after** `turbo` has created the hashes for the repo. In this case, you'd be forced to disable caching for all the tasks whose file inputs might change. That slows your repo down.

Starting with this release, you can now use `$TURBO_JIT$` to defer hashing for selected file inputs until **after** a task's dependencies are complete. In the example below, `check-types` waits for `codegen`, then hashes `src/generated/**` before checking the cache.
...
Tasks using `$TURBO_JIT$` show a deferred hash in `--dry` output until their dependencies finish. [Visit the documentation](/docs/reference/configuration#turbo_jit) to learn more.
