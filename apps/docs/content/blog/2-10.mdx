...
It's common for a repository to have a shutdown or cleanup step to tear down a development environment. Examples include cleanly shutting down a server, terminating a Docker compose script, or closing a background process. Until today, tasks that were meant to gracefully shutdown were not respected. Turborepo would immediately exit, killing all of your tasks instantly.
...
This behavior is zero-congifuration, and if your repository does not have tasks that gracefully shutdown, you will see no observable change. [Visit the documentation](/docs/reference/run#graceful-shutdown) to learn more
