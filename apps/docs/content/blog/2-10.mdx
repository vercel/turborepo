...
date: 2026/06/16
description: "Graceful task shutdown, cache eviction, composable `--affected` and `--filter`, and `$TURBO_JIT$` for generated file inputs"
product: turborepo
type: overview
summary: Release notes for Turborepo 2.10 featuring graceful shutdown, configurable local cache eviction, composable filtering, and just-in-time task input hashing.
tag: "web development"
---
...
Turborepo 2.10 improves day-to-day reliability and control in your monorepo:

- [**Graceful task shutdown**](#graceful-task-shutdown): Interrupt `turbo run` without leaving orphaned processes or truncated logs
- [**Cache eviction with configuration**](#cache-eviction-with-configuration): Automatically reclaim disk space from your local cache
- [**Composable `--affected` and `--filter`**](#composable---affected-and---filter): Narrow CI runs to affected packages within a specific scope
- [**`$TURBO_JIT$` for generated file inputs**](#turbo_jit-for-generated-file-inputs): Hash generated files just in time for correct caching on the first run
...
Pressing `Ctrl+C` during a `turbo run` used to risk orphaned child processes, truncated TUI output, or cache writes that never finished. Turborepo now shuts down tasks gracefully.

On the first `SIGINT` or `SIGTERM`, Turborepo forwards the signal to running task process groups and waits for them to exit. While tasks shut down, Turborepo continues flushing cache writes so you do not lose artifacts from completed work.

If tasks are still running after 3 seconds, Turborepo prints the remaining task list. In an interactive terminal, press `Ctrl+C` again to force shut down. Without a terminal on stdin, Turborepo automatically escalates after 10 seconds.

This works through the Node.js shim as well, so `npx turbo run` and `pnpm turbo run` keep the native `turbo` process alive until shutdown completes.

[Visit the documentation](/docs/reference/run#graceful-shutdown) to learn more.

## Cache eviction with configuration

Local caches can grow without bound as your repository evolves. Turborepo now supports automatic eviction through two opt-in configuration options in `turbo.json`:
...
Eviction runs in a background thread at the start of each `turbo run`, so it does not block task scheduling:

1. **Age-based eviction**: Entries older than `cacheMaxAge` are removed first
2. **Size-based eviction**: If the cache still exceeds `cacheMaxSize`, the oldest remaining entries are removed

Both options accept human-readable values like `"24h"`, `"2w"`, `"500MB"`, and `"1.5GB"`. Set either option to `"0"` to disable it. You can also configure eviction with the [`TURBO_CACHE_MAX_AGE`](/docs/reference/system-environment-variables#turbo_cache_max_age) and [`TURBO_CACHE_MAX_SIZE`](/docs/reference/system-environment-variables#turbo_cache_max_size) environment variables.

[Visit the documentation](/docs/reference/configuration#cachemaxage) to learn more.

## Composable `--affected` and `--filter`

`--affected` and `--filter` were previously mutually exclusive. You can now combine them with intersection semantics to run only the tasks that match **both** constraints.
...
This is especially useful in CI when you want to scope an affected run to a subset of your monorepo, like a single application or package group.

[Visit the documentation](/docs/reference/run#--affected) to learn more.

## `$TURBO_JIT$` for generated file inputs

Tasks that consume generated files can be cached incorrectly on the first run. Without just-in-time hashing, Turborepo hashes file inputs when `turbo` starts — before dependency tasks have created or restored the generated files.

`$TURBO_JIT$` defers hashing for selected file inputs until after a task's `dependsOn` tasks complete:
...
In this example, `check-types` waits for `codegen`, then hashes `src/generated/**` before checking the cache. Tasks using `$TURBO_JIT$` show a deferred hash in `--dry` output until their dependencies finish.

`$TURBO_JIT$` graduates from pre-release status in Turborepo 2.10.

[Visit the documentation](/docs/reference/configuration#turbo_jit) to learn more.
