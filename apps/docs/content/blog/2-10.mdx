...
date: 2026/06/18
description: "Graceful task shutdown, cache eviction, composable `--affected` and `--filter`, and just-in-time task input hashing with `$TURBO_JIT$`"
product: turborepo
type: overview
...
- [**Deferred input hashing with `$TURBO_JIT$`**](#deferred-input-hashing-with-turbo_jit): Hash selected file inputs after upstream tasks complete
...
## Deferred input hashing with `$TURBO_JIT$`
...
Starting with this release, you can use `$TURBO_JIT$` to mark file inputs that should be hashed "just-in-time" - **after** a task's dependencies complete. Code generation is the most common case, but any input that intentionally changes during the run can use this pattern.

In the example below, `check-types` waits for `codegen`, then hashes `src/generated/**` before checking the cache.
