...
TODO: Explain when to use `jit` vs `dependencyOutputs`.

Use `jit` when a task should hash files after another task creates or updates them:
...
Use `dependencyOutputs` when a task should hash declared outputs from a task dependency:
