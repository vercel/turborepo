...
<Date>Tuesday, June 16th, 2026</Date>
...
It's common for a repository to have shutdown or cleanup steps when closing out a development environment. Examples include cleanly shutting down a server, terminating a Docker Compose script, or closing a background process.

In previous versions of Turborepo, tasks that wanted to gracefully shut down were not respected. Turborepo would immediately exit, abruptly killing all of your tasks, and possibly leaving your development environment in an undesirable state. Any tasks that wanted to handle `SIGINT` or `SIGTERM` themselves wouldn't get the opportunity.

Starting with this release, Turborepo will now wait for your tasks to exit Pressing `Ctrl+C` (or sending `SIGINT` or `SIGTERM`) to Turborepo will forward the signals to your tasks. Once all your tasks have completed their work, Turborepo will exit.
...
This feature is zero-configuration, and tasks that do not have their own exit handling will continue to work as before. [Visit the documentation](/docs/reference/run#graceful-shutdown) to learn more

## Deferred input hashing

By default, Turborepo creates the hashes for all of your tasks immediately, right when `turbo` starts. This lets Turborepo get work done as fast as possible, since it can instantly know whether your work is a cache hit or miss.

However, some file inputs are not stable at that point. An upstream task might generate, transform, or otherwise change files that a downstream task depends on **after** `turbo` has already hashed everything. Without a way to defer hashing, you would have to disable caching for those downstream tasks, or risk unexpected cache hits.

Starting with this release, you can use structured `inputs` objects to defer hashing until the right moment in the run:

| Mode | What it does |
| --- | --- |
| `startup` | Hashes files before the task graph starts executing. Use this for the non-deferred part of the cache key. |
| `jit` | Hashes selected files after the task's `dependsOn` tasks complete, just before checking cache. |
| `dependencyOutputs` | Hashes declared `outputs` from selected dependency tasks after those tasks complete. |

Code generation is the most common case for `jit`, but any input that intentionally changes during the run can use this pattern. Use `dependencyOutputs` when a downstream task should hash the artifacts another task declares in `outputs`.

In the example below, `check-types` excludes generated files from startup hashing, waits for `codegen`, then hashes `src/generated/**` before checking the cache.

```jsonc title="turbo.json"
{
  "tasks": {
    "codegen": {
      "inputs": ["schema.graphql", "codegen.config.ts"],
...
Tasks using `jit` or `dependencyOutputs` cannot know their final cache key until execution time, so `turbo run --dry=json` reports `hash: null` with a `hashReason` explaining the deferral.
...
These flags will combine with intersection semantics, only running the tasks that match **both** constraints. [Visit the documentation](/docs/reference/run#--affected) to learn more.

## Local cache eviction

Turborepo saves its local cache into `.turbo/cache`, leaving behind cache artifacts every time you run tasks. While this makes your repository faster locally, there's one problem: The local cache can keep growing forever.

Turborepo now supports automatic eviction through two opt-in configuration options in `turbo.json`:
...
Eviction runs in a background thread at the start of each `turbo run`, so it does not block tasks. You can use either age-based eviction or size-based eviction.

We expect to enable local cache eviction by default in Turborepo 3.0. [Visit the documentation](/docs/reference/configuration#cachemaxage) to learn more.
...
<Accordions>
  ...
  <Accordion title="Feat (13)">
    ...
    - feat: Graceful shutdown ([#12607](https://github.com/vercel/turborepo/pull/12607))
...
We also thank everyone who contributed to this release of Turborepo: @adityasingh2400, @AndyBitz, @Balance8, @bitttttten, @bjormgyg, @christopherkindl, @dancrumb, @DependerKumarSoni, @eastgold15, @gioboa, @haydenbleasel, @JRoy, @kitten, @marc-vercel, @markandrus, @maschwenk, @mattjoll, @molebox, @mvanhorn, @Nsttt, @ognevny, @saiteja-madha, @sleitor, and @Wartijn.
