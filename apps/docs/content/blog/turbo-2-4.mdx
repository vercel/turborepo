...
product: turborepo
type: overview
summary: Release notes for Turborepo 2.4 featuring Boundaries, TUI improvements, and Watch Mode caching.
tag: "web development"
---
...
import { Accordions, Accordion } from "@/components/geistdocs/accordion";
...
A `schema.json` file provides auto-complete and validation in your editor for JSON files. We have a web-accessible version of the `schema.json` for `turbo.json` hosted at [`https://turborepo.dev/schema.json`](https://turborepo.dev/schema.json), but some developers prefer to get the file from `node_modules` to stay synced with the installed version of `turbo`.
