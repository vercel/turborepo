...
pub fn object_assign(args: Vec<JsValue>) -> JsValue {
    if args.iter().all(|arg| matches!(arg, JsValue::Object { .. })) {
        if let Some(mut merged_object) = args.into_iter().reduce(|mut acc, cur| {
            ...
            JsValue::unknown(
                JsValue::call(
                    Box::new(JsValue::WellKnownFunction(WellKnownFunctionKind::ObjectAssign)),
                    vec![],
                ),
...
pub fn path_dirname(mut args: Vec<JsValue>) -> JsValue {
    ...
    JsValue::unknown(
        JsValue::call(
            Box::new(JsValue::WellKnownFunction(WellKnownFunctionKind::PathDirname)),
            args,
        ),
...
pub async fn require_context_require_keys(
    ...
    Ok(if args.is_empty() {
        ...
        JsValue::unknown(
            JsValue::call(
                Box::new(JsValue::WellKnownFunction(WellKnownFunctionKind::RequireContextRequireKeys(
                    val,
                ))),
                args,
            ),
        ...
        if let Some(path) = args[0].as_str() {
            Url::from_file_path(path)
                ...
                .unwrap_or_else(|_| {
                    JsValue::unknown(
                        JsValue::call(
                            Box::new(JsValue::WellKnownFunction(WellKnownFunctionKind::PathToFileUrl)),
                            args,
                        ),
...
fn express(prop: JsValue) -> JsValue {
    match prop.as_str() {
        ...
        _ => JsValue::unknown(
            JsValue::member(
                Box::new(JsValue::WellKnownObject(WellKnownObjectKind::NodeExpressApp)),
                Box::new(prop),
            ),
...
fn protobuf_loader(prop: JsValue) -> JsValue {
    match prop.as_str() {
        ...
        _ => JsValue::unknown(
            JsValue::member(
                Box::new(JsValue::WellKnownObject(WellKnownObjectKind::NodeProtobufLoader)),
                Box::new(prop),
            ),
