...
use turbo_tasks_macros_shared::{
    get_inherent_impl_function_id_ident, get_inherent_impl_function_ident,
...
pub fn value_impl(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    fn trait_value_impl(
        ...
        let trait_ident = get_last_path_ident(trait_path);
        ...
        for item in items.iter() {
            ...
            }) = item
            {
                ...
                trait_registers.push(quote! {
                    value.register_trait_method(<&dyn #trait_path as turbo_tasks::VcValueTrait>::get_trait_type_id(), stringify!(#ident).into(), *#native_function_id_ident);
                });
            }
        }
        ...
        quote! {
            ...
            pub(crate) fn #register(value: &mut turbo_tasks::ValueType) {
                value.register_trait(<&dyn #trait_path as turbo_tasks::VcValueTrait>::get_trait_type_id());
                #(#trait_registers)*
            }

            // NOTE(alexkirsz) We can't have a general `turbo_tasks::Upcast<&dyn Trait> for T where T: Trait` because
            ...
            unsafe impl turbo_tasks::Upcast<&dyn #trait_path> for #ty {}
    ...
    let Some(ty_ident) = ident.map(|ident| Ident::new(&ident.value(), ident.span())).or_else(|| get_type_ident(&*item.self_ty).cloned()) else {
        return quote! {
...
fn get_last_path_ident(path: &Path) -> &Ident {
