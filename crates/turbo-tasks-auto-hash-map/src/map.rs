...
impl<K: Eq + Hash, V, H: BuildHasher + Default> AutoMap<K, V, H> {
        ...
        match self {
            AutoMap::List(list) => match list.entry(key) {
                ...
                vecmap::map::Entry::Vacant(entry) => {
                    Entry::Vacant(VacantEntry::List { this, entry })
                }
            },
    ...
    //     let this = self as *mut Self;
    //     match self {
    //         AutoMap::List(list) => match list.iter().position(|(k, _)| k.borrow()
    // == key) {             Some(index) =>
    // RawEntry::Occupied(OccupiedRawEntry::List { list, index }),
...
pub enum VacantEntry<'a, K, V, H> {
    List {
        this: *mut AutoMap<K, V, H>,
        entry: vecmap::map::VacantEntry<'a, K, V>,
    },
...
// impl<'a, K: Eq + Hash, V, H: BuildHasher + Default> OccupiedRawEntry<'a, K,
// V, H> {     /// see [HashMap::OccupiedEntry::remove](https://doc.rust-lang.org/std/collections/hash_map/enum.OccupiedEntry.html#method.remove)
//     pub fn remove(self) -> V {
//         match self {
//             OccupiedRawEntry::List { list, index } =>
// list.swap_remove(index).1,             OccupiedRawEntry::Map { entry, this }
// => {                 let v = entry.remove();
//                 let this = unsafe { &mut *this };
//                 if this.len() < MIN_HASH_SIZE {
//                     this.convert_to_list();
...
// impl<'a, K: Eq + Hash, V, H: BuildHasher + Default + 'a> VacantRawEntry<'a,
// K, V, H> {     /// see [HashMap::RawVacantEntryMut::insert](https://doc.rust-lang.org/std/collections/hash_map/struct.RawVacantEntryMut.html#method.insert)
