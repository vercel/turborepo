...
pub(super) async fn get_font_fallbacks(
    ...
    match options.adjust_font_fallback {
        AdjustFontFallback::Arial => font_fallbacks.push(FontFallback::Automatic(
        ...
        AdjustFontFallback::TimesNewRoman => font_fallbacks.push(FontFallback::Automatic(
            AutomaticFontFallback {
                scoped_font_family,
    ...
    if let Some(fallback) = &options.fallback {
        font_fallbacks.push(FontFallback::Manual(StringsVc::cell(fallback.clone())));
    }
