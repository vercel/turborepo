...
static UNITS_PER_EM: f32 = 2048.0;
...
async fn get_font_adjustment(
    ...
    match font_res {
        Ok(font) => {
            let az_avg_width = calc_average_width(&font);
            let fallback_avg_width = fallback_font.az_avg_width / fallback_font.units_per_em;
            let size_adjust = match az_avg_width {
                Some(az_avg_width) => {
                    az_avg_width as f64 / UNITS_PER_EM as f64 / fallback_avg_width
                }
                None => 1.0,
            };
            let Some(h_metrics) = font.horizontal_line_metrics(UNITS_PER_EM) else {
                bail!("Unable to calculate horizontal line metrics for font {}", main_descriptor.path);
            };

            Ok(FontAdjustment {
                ascent: h_metrics.ascent as f64 / (UNITS_PER_EM as f64 * size_adjust),
                descent: h_metrics.descent as f64 / (UNITS_PER_EM as f64 * size_adjust),
                line_gap: h_metrics.line_gap as f64 / (UNITS_PER_EM as f64 * size_adjust),
                size_adjust,
            })
        }
        Err(err) => bail!(err),
...
fn calc_average_width(font: &Font) -> Option<f32> {
    let has_all_glyphs = AVG_CHARACTERS
        .chars()
        .all(|c| font.lookup_glyph_index(c) > 0);
    if !has_all_glyphs {
        return None;
    }
    ...
    Some(
        (AVG_CHARACTERS
            .chars()
            .map(|c| font.metrics(c, UNITS_PER_EM).advance_width)
            .sum::<f32>())
...
fn parse_weight_string(weight_str: &str) -> Result<f64> {
    if weight_str == "normal" {
        ...
        let parsed = weight_str.parse::<f64>();
        match parsed {
            Ok(parsed) => Ok(parsed),
            Err(_) => {
                bail!(
