...
//! [`std::process::Command`] type in the standard library, but provides
//! asynchronous versions of functions that create processes. These functions
//! (`spawn`, `status`, `output` and their variants) return "future aware" types
//! that interoperate with Tokio. The asynchronous process support is provided
...
/// [spawn](struct.Command.html#method.spawn), [status](struct.Command.html#
/// method.status), and [output](struct.Command.html#method.output).
///
...
impl Command {
    ...
    ///
    /// ```no_run
    /// tokio_process_pty::Command::new("sh")
    ///   .arg("-C /path/to/repo");
    /// ```
    ///
    /// usage would be:
    ...
    /// Put the new process in its own process group and session as session
    /// leader.
    ///
    /// If a pty is used, it will become the controlling tty of the new session.
    /// Signals sent to the parents' group or session do not reach the new
    /// process.
    ///
    /// # Examples
    ///
    ...
    /// spawned process will not be killed on drop, similar to the behavior
    ...
    ///
    /// By default, stdin, stdout and stderr are inherited from the parent.
    /// If any input/output handles are set to a pipe then they will be
    /// immediately closed after the child is spawned.
    ///
    /// All I/O this child does will be associated with the current default
    /// event loop.
    ///
...
/// A drop guard which can ensure the child process is killed on drop if
/// specified.
#[derive(Debug)]
...
///
/// This type implements the `AsyncWrite` trait to pass data to the stdin handle
/// of handle of a child process asynchronously.
#[derive(Debug)]
