...
impl<'a> EngineBuilder<'a> {
    ...
    fn has_task_definition_in_run(
        ...
        Self::has_task_definition_in_run_inner(
            loader,
            workspace,
            task_name,
            task_id,
    ...
    fn has_task_definition_in_run_inner(
        ...
        for extend in turbo_json.extends.as_inner().iter() {
            ...
            if Self::has_task_definition_in_run_inner(
                loader,
                &extend_package,
                task_name,
                task_id,
    ...
    fn collect_tasks_from_extends_chain(
        ...
        let turbo_json = match loader.load(workspace) {
            ...
            Err(config::Error::NoTurboJSON) if !matches!(workspace, PackageName::Root) => {
                ...
                return Self::collect_tasks_from_extends_chain(
                    loader,
                    &PackageName::Root,
                    tasks,
                    visited,
        ...
        // Add all tasks from this turbo.json
        tasks.extend(turbo_json.tasks.keys().cloned());

        // Follow the extends chain
        for extend in turbo_json.extends.as_inner().iter() {
            let extend_package = PackageName::from(extend.as_str());
            Self::collect_tasks_from_extends_chain(loader, &extend_package, tasks, visited)?;
        }
        ...
        // the extends chain will eventually reach root.
        if turbo_json.extends.is_empty() && !matches!(workspace, PackageName::Root) {
            Self::collect_tasks_from_extends_chain(loader, &PackageName::Root, tasks, visited)?;
        }
