...
pub struct BuildManifestChunkAsset {
    ...
    pub chunking_context: BuildChunkingContextVc,
...
#[turbo_tasks::function]
fn build_manifest_chunk_reference_description() -> StringVc {
    StringVc::cell("build manifest chunk".to_string())
}
    ...
    async fn references(self_vc: BuildManifestChunkAssetVc) -> Result<AssetReferencesVc> {
        ...
        Ok(AssetReferencesVc::cell(
            chunks
                ...
                .copied()
                .map(|chunk| {
                    SingleAssetReferenceVc::new(chunk, build_manifest_chunk_reference_description())
                        .into()
                })
                .collect(),
        ))
...
impl EcmascriptChunkPlaceable for BuildManifestChunkAsset {
    ...
    async fn as_chunk_item(
        ...
        let Some(context) = BuildChunkingContextVc::resolve_from(context).await? else {
            bail!("invalid chunking context");
        };

        Ok(BuildManifestChunkItem {
            context,
            manifest: self_vc,
        }
        .cell()
