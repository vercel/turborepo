//! Shim module for turborepo-cli.
//!
//! This module provides the integration between the `turborepo-shim` crate and
//! `turborepo-cli`. It implements the traits required by the shim.
...
/// Implementation of `TurboRunner` that calls into `turborepo-cli`'s CLI.
...
/// Implementation of `ConfigProvider` that uses `turborepo-cli`'s configuration
/// system.
...
/// Implementation of `ChildSpawner` that uses `turborepo-cli`'s spawn_child
/// function.
