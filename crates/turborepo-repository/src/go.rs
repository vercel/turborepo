...
pub enum Error {
    ...
    #[error(
        "exact subprocess-free Go planning is unavailable at {path}: {reason}. This requires \
         native metadata beyond the current static planner, which will not run `go` or guess \
         tasks and edges. Simplify the source constraints or use version-independent local \
         replacements. Installing Go alone does not add the missing native-metadata planning \
         phase."
    )]
    StaticDiscoveryUnavailable { path: String, reason: String },
    #[error(
        "Go prune planning is unavailable: {reason}. The exact prune domain comes from full Go \
         discovery; static planning refuses to plan against partial facts."
    )]
    PrunePlanningDeferred { reason: String },
    #[error(transparent)]
...
struct GoPruneKnowledge {
    ...
    /// Why planning-time prune planning must refuse instead of planning:
    /// a static observation whose relationship closure is deferred, or whose
    /// `go.work` lacks the `go` directive the pruned file needs. Always
    /// `None` on the authoritative observation, whose facts are exact.
...
impl PruneDomain for GoPruneKnowledge {
    ...
    fn plan(
        ...
        if let Some(reason) = &self.deferred {
            // Planning against partial facts would silently under-retain the
            // dependency closure; preparation owns the exact domain instead.
            return Err(crate::prune_knowledge::Error::Failed(Box::new(
                Error::PrunePlanningDeferred {
                    reason: reason.clone(),
                },
            )));
        }
...
impl GoContributor {
    ...
    fn anchored_definition(&self, path: &str) -> Result<AnchoredSystemPathBuf, Error> {
        Ok(AnchoredSystemPathBuf::from_raw(path)?)
    }

    /// Planning discovery: reconstruct module membership, task catalogues, and
    /// internal edges by parsing `go.work`/`go.mod` and Go sources without
    /// invoking `go` (see [`static_discovery`]).
    ///
    /// Module identities, task names, and internal edges match
    /// [`Self::discover_packages`] exactly. Only target OS and hash I/O remain
    /// provisional: `go env` is unavailable without a subprocess, so the host
    /// OS stands in for `GOOS`, cache prefixes are empty, and external
    /// resolution is reported `Unavailable`. The observation is replaced by
    /// the authoritative path before hashing because
    /// [`RepositoryContributor::requires_preparation`] remains `true`. When a
    /// fact is impossible to prove without `go` — for example a
    /// build-constrained `main` package, or a version-sensitive local
    /// replacement that active remote requirements could upgrade — the member
    /// that owns the sources keeps every provable fact and reports the rest
    /// through a scoped [`toolchain::PlanningUncertainty`] instead of guessing
    /// or failing planning for unrelated toolchains. Core refuses only the
    /// selections that consult an unproven fact, and resolves the records by
    /// preparing this toolchain when one of its own tasks is selected. The
    /// observation also contributes the `go` prune domain its scopes' task
    /// contracts reference, built from static facts; it refuses to plan when
    /// its retention closure is not provable, preparation swaps it for the
    /// authoritative domain, and the eager prune path always uses full
    /// discovery's.
    fn discover_packages_statically_inner(&self) -> Result<DiscoveredPackages, Error> {
        let workspace = turborepo_rayon_compat::block_in_place(|| {
            static_discovery::discover_workspace_statically(&self.repo_root)
        })?;
        ...
        let change_observation = go_change_observation(&self.repo_root, &workspace.modules, &[])?;
        ...
        // `go env` is unavailable without a subprocess. The host OS is Go's
        // default target; preparation replaces this before any hash reads it,
        // and task names never depend on it.
        ...
        let mut packages = workspace
            .modules
            .iter()
            .map(|module| package_from_module(module, target_os, &[], ""))
            .collect::<Vec<_>>();
        packages.push(
            ...
            .with_task_contract(crate::task_contracts::ScopeTaskContract::go(
                GoTaskContract::workspace(target_os, &[]),
            )),
        );

        let mut members = module_names;
        members.push(GO_WORKSPACE_NAME.to_string());
        let mut definition_sources = vec![self.anchored_definition(GO_WORK)?];
        if self.repo_root.join_component(GO_WORK_SUM).exists() {
            definition_sources.push(self.anchored_definition(GO_WORK_SUM)?);
        }
        ...
        let resolution = ExternalResolutionDomain::new(
            ...
            ExternalResolutionData::Unavailable(ResolutionUnavailableReason::new(
                "go-preparation-required",
                "Go workspace resolution requires preparation; full discovery replaces this \
                 observation before hashing.",
            )),
        );

        // Scoped facts this observation could not prove travel with it instead
        // of failing planning: the records are scoped to member identities this
        // same observation contributes, and full discovery — which replaces
        // this observation before hashing — resolves every fact, so it never
        // attaches any.
        let mut discovered = DiscoveredPackages::new(packages, workspace_roots)
        ...
        // The static prune domain satisfies every Go scope's contract
        // reference; preparation swaps it for the authoritative one. The
        // domain refuses to plan when its retention closure is not provable,
        // and the eager prune path always uses full discovery's domain.
        if let Some(prune) = workspace.prune {
            discovered = discovered.with_prune_domain(std::sync::Arc::new(prune));
        }
        Ok(discovered)
    }
}
...
mod tests {
    ...
    #[test]
    fn static_discovery_unavailable_explains_exact_planning() {
        let error = Error::StaticDiscoveryUnavailable {
            path: "/repo/go.work".to_string(),
            reason: "example.com/alias has a version-sensitive local replacement and an active \
                     remote requirement"
                .to_string(),
        };
        let diagnostic = error.to_string();

        // This is a missing planning capability, not a missing executable:
        // installing Go must not be presented as a bypass for static discovery.
        assert!(diagnostic.contains("/repo/go.work"));
        assert!(diagnostic.contains("version-sensitive"));
        assert!(diagnostic.contains("subprocess-free"));
        assert!(diagnostic.contains("native metadata"));
        assert!(diagnostic.contains("will not run `go`"));
    }
    ...
    // Static discovery must not invoke `go`, so these tests deliberately do not
    // guard on `go_available`. Differential tests that compare the static
    // scanner against the authoritative `go` commands are guarded separately.
    fn static_contributor(root: &AbsoluteSystemPath) -> std::sync::Arc<GoContributor> {
        GoContributor::new(
            AbsoluteSystemPathBuf::new(root.as_str()).expect("test root is absolute"),
        )
    }

    fn static_packages(root: &AbsoluteSystemPath) -> DiscoveredPackages {
        static_contributor(root)
            .discover_packages_statically_inner()
            .expect("static discovery succeeds")
    }

    fn static_discovery_error(root: &AbsoluteSystemPath) -> Error {
        static_contributor(root)
            .discover_packages_statically_inner()
            .expect_err("static discovery fails")
    }

    fn static_module_edges(discovered: DiscoveredPackages) -> HashMap<String, Vec<String>> {
        let (packages, _, _, _, _) = discovered.into_parts();
        let mut edges = HashMap::new();
        for package in packages {
            let parts = package.into_parts();
            if parts.scope_kind == crate::toolchain::DiscoveredScopeKind::Aggregate {
                continue;
            }
            let name = parts.name.expect("native module has a name");
    ...
    fn overwrite_workspace(root: &AbsoluteSystemPath, contents: &str) {
        root.join_component(GO_WORK)
            .create_with_contents(contents)
            .unwrap();
    }

    /// Canonicalize a temporary root onto the path `go` reports: `go list`
    /// emits canonical directories, while a tempdir can sit behind a symlink
    /// (for example `/var` -> `/private/var` on macOS). Differential tests must
    /// compare identical paths, not path aliases.
    fn canonical_temp_root(tempdir: &tempfile::TempDir) -> AbsoluteSystemPathBuf {
        AbsoluteSystemPathBuf::try_from(tempdir.path())
            .unwrap()
            .to_realpath()
            .unwrap()
    ...
    /// Write a Go source file (and any missing parent directories) below
    /// `root`.
    fn write_source(root: &AbsoluteSystemPath, relative: &str, contents: &str) {
        let path = join_relative_path(root, relative).unwrap();
        path.parent().unwrap().create_dir_all().unwrap();
        path.create_with_contents(contents).unwrap();
    }

    /// Internal edges reported by authoritative discovery, in the same shape as
    /// [`static_module_edges`].
    fn full_module_edges(workspace: &DiscoveredWorkspace) -> HashMap<String, Vec<String>> {
        workspace
            .modules
            .iter()
            .map(|module| {
                let mut targets = module
                    ...
                    .collect::<Vec<_>>();
                targets.sort();
                (module.module_path.clone(), targets)
            })
            .collect()
    }

    #[test]
    fn static_discovery_reports_membership_tasks_and_unavailable_resolution() {
        let tempdir = tempfile::tempdir().unwrap();
        // Canonicalized: the `dev` target strips the canonical directory the
        // `go` command reports against the module directory it is given, so
        // a tempdir behind a symlink would drop `dev` exactly as the
        // authoritative helper does — this fixture exercises the target
        // being present.
        let root = canonical_temp_root(&tempdir);
        write_workspace(
            &root,
            &[
                ("apps/api", "example.com/api", ""),
        ...
        write_source(&root, "apps/api/main.go", "package main\nfunc main() {}\n");
        write_source(&root, "packages/lib/lib.go", "package lib\n");

        let discovered = static_packages(&root);
        assert!(
            discovered.planning_uncertainties().is_empty(),
            "an unambiguous workspace proves every planning fact"
        );
        let (packages, workspace_roots, resolutions, observations, prune) = discovered.into_parts();

        assert_eq!(workspace_roots.len(), 1);
        assert_eq!(workspace_roots[0].kind(), "go");
        // The static observation contributes the prune domain its scopes'
        // task contracts reference; a graph built from it validates.
        assert_eq!(prune.len(), 1, "static planning contributes the go domain");
        assert_eq!(prune[0].id().to_string(), "go");
        assert_eq!(
            observations.len(),
            1,
            "static change observation is emitted"
        );
        ...
        for package in packages {
            let parts = package.into_parts();
            let name = parts.name.clone().expect("scope has a name");
            let tasks = parts
                .native_tasks
                .clone()
                .unwrap_or_default()
                ...
                .collect::<Vec<_>>();
            if parts.scope_kind == crate::toolchain::DiscoveredScopeKind::Aggregate {
                assert_eq!(name, GO_WORKSPACE_NAME);
                assert_eq!(tasks, ["test", "lint", "format", "build"]);
            } else {
                module_names.push(name.clone());
                // Matching on the name keeps the branches disjoint, so
                // `tasks` moves into exactly one of them; two sequential
                // `if`s would move it twice.
                match name.as_str() {
                    "example.com/api" => api_tasks = tasks,
                    "example.com/lib" => lib_tasks = tasks,
                    _ => {}
                }
            }
        }
        module_names.sort();
        assert_eq!(module_names, ["example.com/api", "example.com/lib"]);
        // Static planning classifies packages from source, so the catalogues
        // are exact: `dev` appears only for the module with an unambiguous
        // `main` package.
        assert_eq!(api_tasks, ["build", "test", "lint", "format", "dev"]);
        assert_eq!(lib_tasks, ["build", "test", "lint", "format"]);

        assert_eq!(resolutions.len(), 1);
        match resolutions[0].data() {
            ExternalResolutionData::Unavailable(reason) => {
                assert_eq!(reason.code(), "go-preparation-required");
            }
            other => panic!("expected unavailable resolution, got {other:?}"),
        }
        let mut members = resolutions[0].members().to_vec();
        members.sort();
        assert_eq!(
            members,
            ["example.com/api", "example.com/lib", GO_WORKSPACE_NAME]
        );
    }

    #[test]
    fn static_discovery_workspace_replacements_override_module_replacements() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/api"], ["example.com/lib"]);
    }

    #[test]
    fn static_discovery_matches_exact_versions_and_ignores_provably_lower_ones() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
    ...
    #[test]
    fn static_discovery_selects_only_the_required_version_replacement() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
            &[
                (
                    ...
                    "module example.com/api\n\ngo 1.22\n\nrequire example.com/alias \
                     v1.0.0\n\nreplace (\n\texample.com/alias v1.0.0 => \
                     ../lib\n\texample.com/alias v1.2.0 => ../extra\n)\n",
                ),
                ("lib", "example.com/lib", ""),
        ...
        // Nothing in the closed workspace requires `alias` past v1.0.0, so MVS
        // never selects v1.2.0 and the unused replacement adds no edge.
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/api"], ["example.com/lib"]);
        assert_eq!(edges["example.com/extra"], Vec::<String>::new());
    }

    #[test]
    fn static_discovery_applies_the_maximum_required_version_across_roots() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
            &[
                ...
                (
                    "other",
                    "example.com/other",
                    "module example.com/other\n\ngo 1.22\n\nrequire example.com/alias v1.2.0\n",
                ),
                ("lib", "example.com/lib", ""),
                ("extra", "example.com/extra", ""),
            ],
        ...
        // The workspace maximum for `alias` is v1.2.0, required by the other
        // root. Replacements apply workspace-wide, so the v1.2.0 replacement
        // resolves every requirer's edge and the v1.0.0 replacement is inactive.
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/api"], ["example.com/extra"]);
        assert_eq!(edges["example.com/other"], ["example.com/extra"]);
        assert_eq!(edges["example.com/lib"], Vec::<String>::new());
    }

    #[test]
    fn static_discovery_evaluates_replacement_precedence_per_version() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        overwrite_workspace(
            &root,
            "go 1.22\nuse (\n./api\n./lib\n./extra\n)\n\nreplace example.com/alias v1.2.0 => \
             ./extra\n",
        );

        // The `go.work` replacement names only v1.2.0, so it does not suppress
        // the member's wildcard replacement for the required v1.0.0. Nothing in
        // the closed workspace upgrades `alias` past v1.0.0, so the workspace
        // replacement never applies and only `lib` is reachable.
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/api"], ["example.com/lib"]);
        assert_eq!(edges["example.com/extra"], Vec::<String>::new());
    }
    ...
    #[test]
    fn static_discovery_workspace_wildcard_shadows_module_exact() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
            &[
                (
                    ...
                    "example.com/api",
                    "module example.com/api\n\ngo 1.22\n\nrequire example.com/alias \
                     v1.0.0\n\nreplace example.com/alias v1.0.0 => ../lib\n",
                ),
                ("lib", "example.com/lib", ""),
        ...
        overwrite_workspace(
            &root,
            "go 1.22\nuse (\n./api\n./lib\n./other\n)\n\nreplace example.com/alias => ./other\n",
        );

        // The workspace wildcard outranks the member's exact version for every
        // version, so only the workspace target is reachable.
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/api"], ["example.com/other"]);
    }

    #[test]
    fn static_discovery_applies_replacements_from_other_main_modules() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
            &[
                ...
                (
                    "declarer",
                    "example.com/declarer",
                    "module example.com/declarer\n\ngo 1.22\n\nreplace example.com/alias => \
                     ../provider\n",
                ),
            ],
        );

        // Replacements from every main module apply workspace-wide, so a
        // replacement declared outside the requirer still creates the edge.
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/app"], ["example.com/provider"]);
    }

    #[test]
    fn static_discovery_excludes_provably_lower_replacement_versions() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        // MVS never selects below a direct requirement, so the v1.0.0
        // replacement cannot apply and must add no edge.
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/api"], Vec::<String>::new());
    }

    #[test]
    fn static_discovery_validates_unused_workspace_local_replacements() {
        // A `go.work` local replacement that no member requires is still
        // validated, matching `discover_workspace`'s prune knowledge.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(&root, &[("api", "example.com/api", "")]);
        overwrite_workspace(
            &root,
            "go 1.22\nuse (\n./api\n)\n\nreplace example.com/unused => ./missing\n",
        );
        assert!(matches!(
            static_discovery_error(&root),
            Error::MissingGoMod { .. }
        ));

        // A versioned remote replacement stays external and is accepted.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(&root, &[("api", "example.com/api", "")]);
        overwrite_workspace(
            &root,
            "go 1.22\nuse (\n./api\n)\n\nreplace example.com/unused => example.com/fork v1.0.0\n",
        );
        let edges = static_module_edges(static_packages(&root));
        assert_eq!(edges["example.com/api"], Vec::<String>::new());
    }
    ...
    #[test]
    fn static_discovery_does_not_emit_self_edges() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
    ...
    #[test]
    fn static_discovery_preserves_membership_validation() {
        ...
        assert!(matches!(
            static_discovery_error(&root),
            Error::DuplicateModuleIdentity { .. }
        ));

        // Member outside the repository.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        overwrite_workspace(&root, "go 1.22\n\nuse ../outside\n");
        assert!(matches!(
            static_discovery_error(&root),
            Error::MemberOutsideRepository { .. }
        ));
        ...
        assert!(matches!(
            static_discovery_error(&root),
            Error::VendoredModule { .. }
        ));

        // Reserved aggregate name.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(&root, &[("module", GO_WORKSPACE_NAME, "")]);
        assert!(matches!(
            static_discovery_error(&root),
            Error::WorkspaceNameCollision { .. }
        ));
        ...
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(&root, &[("module", "example.com/module", "go 1.22\n")]);
        assert!(matches!(
            static_discovery_error(&root),
            Error::MissingModulePath { .. }
        ));
    }

    #[cfg(unix)]
    #[test]
    fn static_discovery_rejects_member_symlink_outside_repository() {
        use std::os::unix::fs::symlink;
    ...
    #[test]
    fn static_discovery_validates_local_replacements_without_go() {
        // Outside-repository local replacement.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
            &[(
                ...
                "example.com/api",
                "module example.com/api\n\ngo 1.22\n\nrequire example.com/alias v1.0.0\n\nreplace \
                 example.com/alias => ../../outside\n",
            )],
        );
        assert!(matches!(
            static_discovery_error(&root),
            Error::OutsideRepositoryLocalModule { .. }
        ));

        // Missing local replacement directory.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        // Local replacement that is not a tracked member.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        root.join_components(&["copy"]).create_dir_all().unwrap();
        root.join_components(&["copy", GO_MOD])
            .create_with_contents("module example.com/lib\n\ngo 1.22\n")
            .unwrap();
        assert!(matches!(
            static_discovery_error(&root),
            Error::NonMemberLocalModule { .. }
        ));
    }

    #[test]
    fn static_discovery_matches_full_membership_on_a_valid_workspace() {
        if !go_available() {
            return;
        }
        let tempdir = tempfile::tempdir().unwrap();
        ...
        let full = discover_workspace(&root).expect("authoritative discovery succeeds");
        let static_modules = static_contributor(&root)
            .discover_packages_statically_inner()
            .unwrap();
        let (packages, _, _, _, _) = static_modules.into_parts();
        let mut static_names = packages
            .into_iter()
            .filter_map(|package| {
                let parts = package.into_parts();
                (parts.scope_kind == crate::toolchain::DiscoveredScopeKind::Package)
                    .then_some(parts.name.expect("named module"))
            })
            .collect::<Vec<_>>();
        static_names.sort();
        let mut full_names = full
            ...
            .collect::<Vec<_>>();
        full_names.sort();
        assert_eq!(static_names, full_names);
    }

    #[test]
    fn static_task_catalogue_matches_prepared_tasks_exactly() {
        if !go_available() {
            return;
        }
        let tempdir = tempfile::tempdir().unwrap();
        // Canonicalized so the directories `go list` reports and the fixture
        // agree even when the tempdir sits behind a symlink (for example
        ...
        write_workspace(
            ...
            &[
                ("apps/api", "example.com/api", ""),
                ("apps/service", "example.com/service", ""),
                ("packages/lib", "example.com/lib", ""),
            ],
        );
        write_source(&root, "apps/api/main.go", "package main\nfunc main() {}\n");
        ...
        let prepared = full
            ...
            .collect::<HashMap<_, _>>();
        assert_eq!(
            prepared["example.com/api"],
            ["build", "test", "lint", "format", "dev"]
        );
        assert_eq!(
            prepared["example.com/service"],
            ["build", "test", "lint", "format", "dev"]
        );
        assert_eq!(
            prepared["example.com/lib"],
            ["build", "test", "lint", "format"]
        );

        let (packages, _, _, _, _) = static_packages(&root).into_parts();
        ...
        for package in packages {
            ...
            if parts.scope_kind == crate::toolchain::DiscoveredScopeKind::Aggregate {
                ...
                planned.insert(parts.name.expect("native module has a name"), tasks);
            }
        }
        assert_eq!(planned, prepared, "planning catalogues are exact");
        assert_eq!(aggregate_tasks, ["test", "lint", "format", "build"]);
    }

    #[test]
    fn static_runnable_target_matches_go_list_exactly() {
        if !go_available() {
            return;
        }
        // Verified `go list` classifications that the authoritative helper and
        // the static scanner must reproduce identically.
        let cases: &[(&str, &[(&str, &str)], Option<&str>)] = &[
            ("library module", &[("lib.go", "package lib\n")], None),
            ("module without sources", &[], None),
            (
            ...
            (
                "several files in one main package",
                &[
                    ("main.go", "package main\nfunc main() {}\n"),
            ...
            (
                ...
                &[
                    ("lib.go", "package lib\n"),
                    ("_underscored.go", "package main\n"),
                    (".dotted.go", "package main\n"),
                ],
                None,
            ),
            (
                ...
                &[
                    ("main.go", "package main\nfunc main() {}\n"),
                    (".hidden/x.go", "package main\n"),
                    ("_hidden/x.go", "package main\n"),
                    ("testdata/x.go", "package main\n"),
                    // `go list ./...` ignores paths with a `vendor` element
                    // anywhere except the last: a package *named* `vendor`
                    // is matched, while nothing beneath it is. The main
                    // package sits below `vendor` so the tree is excluded.
                    ("vendor/direct.go", "package lib\n"),
                    ("vendor/x/x.go", "package main\n"),
                    ("nested/go.mod", "module example.com/nested\n\ngo 1.22\n"),
                    ("nested/x.go", "package main\n"),
                ],
                Some("."),
            ),
            (
                "comments are not package clauses",
                &[(
                    "lib.go",
                    "// package main is only a comment.\npackage lib\n",
                )],
                None,
            ),
            (
            ...
            (
                "library with its test package",
                &[
                    ("lib.go", "package lib\n"),
                    ("lib_test.go", "package lib_test\n"),
                ],
                None,
            ),
            (
                "test file with an incompatible package",
                &[
                    ("lib.go", "package lib\n"),
                    ("lib_test.go", "package other\n"),
                ],
                None,
            ),
        ];
        ...
        for (name, files, expected) in cases {
            let tempdir = tempfile::tempdir().unwrap();
            // Canonicalized: `go list` reports canonical directories while the
            // tempdir may sit behind a symlink, and the authoritative helper
            // strips the directory it was given. Without canonicalization the
            // comparison would measure a path alias, not classification.
            let root = canonical_temp_root(&tempdir);
            root.join_component(GO_MOD)
                .create_with_contents("module example.com/m\n\ngo 1.22\n")
                .unwrap();
            for (relative, contents) in *files {
                write_source(&root, relative, contents);
            }

            let authoritative = runnable_target(&root).unwrap();
            assert_eq!(
                authoritative,
                expected.map(str::to_string),
                "{name}: go list"
            );
            let planned = static_discovery::runnable_target_statically(&root)
                .unwrap_or_else(|error| panic!("{name}: static classification failed: {error}"));
            assert_eq!(planned, authoritative, "{name}: static scanner");
        }
    }

    #[cfg(unix)]
    #[test]
    fn static_runnable_target_excludes_symlinked_directories() {
        if !go_available() {
            return;
        }
        use std::os::unix::fs::symlink;
        ...
        root.join_component(GO_MOD)
            .create_with_contents("module example.com/m\n\ngo 1.22\n")
            .unwrap();
        write_source(&root, "main.go", "package main\nfunc main() {}\n");
        let outside = tempfile::tempdir().unwrap();
        let outside_root = AbsoluteSystemPathBuf::try_from(outside.path()).unwrap();
        write_source(&outside_root, "x.go", "package main\n");
        symlink(
            outside_root.as_std_path(),
            root.join_component("linked").as_std_path(),
        )
        .unwrap();

        // `go list ./...` does not traverse the symlinked directory, so the
        // root `main` package remains the only one; the static scanner must
        // exclude it the same way.
        let authoritative = runnable_target(&root).unwrap();
        assert_eq!(authoritative, Some(".".to_string()));
        assert_eq!(
            static_discovery::runnable_target_statically(&root).unwrap(),
            authoritative
        );
    }

    #[test]
    fn static_runnable_target_refuses_to_guess_build_constrained_mains() {
        let tempdir = tempfile::tempdir().unwrap();
        let root = canonical_temp_root(&tempdir);
        root.join_component(GO_MOD)
            .create_with_contents("module example.com/m\n\ngo 1.22\n")
            .unwrap();
        write_source(&root, "lib.go", "package lib\n");
        write_source(
            &root,
            "cmd/service/main.go",
        ...
        // Whether `./cmd/service` is the sole `main` package depends on the
        // `integration` build tag, which the scanner cannot sample without `go`
        // (it may be set through GOFLAGS). Exact planning is unavailable, and
        // the scanner must say so instead of guessing either catalogue.
        assert!(matches!(
            static_discovery::runnable_target_statically(&root),
            Err(Error::StaticDiscoveryUnavailable { .. })
        ));

        if go_available() {
            // With default tags the constrained file is excluded, so Go
            // classifies the module as a library with no `dev` task.
            assert_eq!(runnable_target(&root).unwrap(), None);
        }
    }

    #[test]
    fn static_discovery_matches_the_full_internal_graph_on_closed_replacements() {
        if !go_available() {
            return;
        }
        // The unused v1.2.0 replacement must not leak an edge: nothing in the
        // closed workspace requires `alias` past v1.0.0. The root is
        // canonicalized so the authoritative path's `go list -m all`
        // directories — which `go` reports canonically — stay inside the
        // repository even when the tempdir sits behind a symlink.
        let tempdir = tempfile::tempdir().unwrap();
        let root = canonical_temp_root(&tempdir);
        write_workspace(
            &root,
        ...
        let edges = static_module_edges(static_packages(&root));
        let full = full_module_edges(&discover_workspace(&root).unwrap());
        assert_eq!(edges, full);
        assert_eq!(edges["example.com/api"], ["example.com/lib"]);
        assert_eq!(edges["example.com/extra"], Vec::<String>::new());

        // Raising the workspace maximum to v1.2.0 selects `extra` for every
        // requirer, again matching the full graph exactly.
        let tempdir = tempfile::tempdir().unwrap();
        let root = canonical_temp_root(&tempdir);
        write_workspace(
            &root,
    ...
    #[test]
    fn static_discovery_defers_remote_sensitive_edges_to_their_requiring_scope() {
        // An active remote requirement can upgrade `alias` through transitives
        // that no static parse can see; at v1.2.0 the replacement target
        // changes. The requirer keeps every provable fact — identity, and no
        // guessed edge for `alias` — and reports one scoped record whose
        // possible targets are exactly the candidates the edge could still
        // connect to. Unrelated scopes are planned normally.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        let discovered = static_packages(&root);
        // The deferred edge contributes nothing: `api`'s provable edge set is
        // empty, and the untouched members keep theirs.
        let edges = static_module_edges(discovered.clone());
        assert_eq!(edges["example.com/api"], Vec::<String>::new());

        let uncertainties: Vec<_> = discovered.planning_uncertainties().to_vec();
        assert_eq!(uncertainties.len(), 1, "exactly the requirer reports");
        let uncertainty = &uncertainties[0];
        assert_eq!(uncertainty.scope(), "example.com/api");
        assert_eq!(uncertainty.code(), "ambiguous-remote-replacement");
        assert_eq!(
            uncertainty.kind(),
            crate::toolchain::PlanningUncertaintyKind::InternalEdges
        );
        assert_eq!(
            uncertainty.possible_targets(),
            Some(
                [
            ...
            "the bound is the selected target plus every upgrade candidate"
        );
        assert!(uncertainty.message().contains("example.com/alias"));
    }

    #[test]
    fn static_discovery_rejects_conflicting_replacements_like_the_go_command() {
        // Two members replace the same path with different local targets. The
        // `go` command itself rejects this workspace — `go work sync`, `go mod
        // graph`, and `go list -m all` all fail with "conflicting
        // replacements" under every environment (verified against go 1.26),
        // and agreeing duplicates are accepted — so static discovery fails
        // structurally rather than deferring: full discovery fails the same
        // way, and no uncertainty bound could honestly say where the edge
        // connects when nothing does.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        let error = static_discovery_error(&root);
        assert!(matches!(error, Error::MalformedReplacement { .. }));
        let diagnostic = error.to_string();
        assert!(diagnostic.contains("example.com/alias"));
        assert!(diagnostic.contains("conflicting"));
    }

    #[test]
    fn static_discovery_defers_constrained_runnable_targets_as_catalogue_uncertainty() {
        // A build-constrained `main` package cannot be classified without
        // `go`, so the module contributes no `dev` task — never a guessed
        // one — and the catalogue record covers `build` too, whose
        // entrypoint, outputs, and cacheability follow the same unproven
        // target. Edges and identities stay exact.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
        ...
        write_source(&root, "api/lib.go", "package lib\n");
        write_source(
            &root,
        ...
        let discovered = static_packages(&root);
        let edges = static_module_edges(discovered.clone());
        // The member require stays exact; only the runnable target defers.
        assert_eq!(edges["example.com/api"], ["example.com/lib"]);
        assert_eq!(edges["example.com/lib"], Vec::<String>::new());
        ...
        let uncertainty = &uncertainties[0];
        assert_eq!(uncertainty.scope(), "example.com/api");
        assert_eq!(uncertainty.code(), "constrained-runnable-target");
        assert_eq!(
            uncertainty.kind(),
            crate::toolchain::PlanningUncertaintyKind::TaskCatalogue
        );
        assert_eq!(
            uncertainty.uncertain_task_names(),
            ["dev".to_string(), "build".to_string()].as_slice()
        );
        assert!(uncertainty.message().contains("runnable target"));

        // No guessed `dev` task: the module's catalogue is the provable
        // subset, and `build` is contributed in its library shape.
        let (packages, _, _, _, _) = discovered.into_parts();
        for package in packages {
            let parts = package.into_parts();
            if parts.scope_kind == crate::toolchain::DiscoveredScopeKind::Aggregate {
            ...
            assert_eq!(
                tasks,
                ["build", "test", "lint", "format"],
                "the deferred target contributes no `dev` task"
            );
        }
    }

    #[test]
    fn static_discovery_keeps_stable_wildcard_replacements_with_remote_requirements() {
        // A wildcard replacement applies at every version, so no remote
        // transitive upgrade can change the target: the edge stays exact even
        // with an active remote requirement.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        write_workspace(
            &root,
    ...
    #[test]
    fn static_catalogue_is_exact_for_a_library_beside_a_root_dev_script() {
        // Regression for repositories whose root `dev` task comes from the root
        // JavaScript package.json: the Go library member must not grow a
        // matching `dev`, its catalogue is exactly the four Go tasks, and the
        // workspace aggregate is unchanged.
        let tempdir = tempfile::tempdir().unwrap();
        let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap();
        root.join_component("package.json")
            ...
            .unwrap();
        write_workspace(&root, &[("packages/lib", "example.com/lib", "")]);
        write_source(&root, "packages/lib/lib.go", "package lib\n");
        ...
        assert_eq!(module_tasks, ["build", "test", "lint", "format"]);
        assert_eq!(aggregate_tasks, ["test", "lint", "format", "build"]);
    }

    #[tokio::test]
    async fn staged_graph_accepts_pure_go_library_workspace_without_subprocesses() {
        // Regression for the CLI failure "scope references unavailable prune
        // domain go": every Go scope's task contract references the go prune
        // domain, so a static observation that contributes scopes without a
        // domain made the whole staged graph invalid. The static observation
        // now carries the domain built from static facts — no `go` process,
        // no package.json, no JavaScript — and it plans truthfully for exact
        // workspaces, while preparation still owns the authoritative domain.
        let tempdir = tempfile::tempdir().unwrap();
        let root = canonical_temp_root(&tempdir);
        write_workspace(&root, &[("lib", "example.com/lib", "")]);
        write_source(&root, "lib/lib.go", "package lib\n");

        let staged = crate::package_graph::PackageGraph::builder_optional(&root, None)
            .with_go()
            .build_staged()
            .await
            .unwrap();
        ...
        // The pure-Go repository is planned: the library scope is accepted
        // with its native task context, and nothing deferred.
        assert!(
            planning
                .package_task_context(&crate::package_graph::PackageName::from("example.com/lib"))
                .is_some(),
            "the Go library scope is planned"
        );
        assert_eq!(planning.planning_uncertainties().count(), 0);

        // The referenced prune domain is retained, and the static domain
        // plans exactly what the authoritative one would render for this
        // closed workspace — computed without invoking `go`.
        let domains: Vec<_> = planning.prune_domains().map(ToString::to_string).collect();
        assert!(domains.contains(&"go".to_string()), "domains: {domains:?}");
        let plan = planning
            .prune_plan(
            ...
            .unwrap()
            .expect("the exact workspace plans");
        assert_eq!(
            plan.root_files,
            vec![(
                "go.work".to_string(),
            ...
            "the pruned go.work is rendered from static facts alone"
        );
        assert!(plan.extra_packages.is_empty());
        assert_eq!(
            plan.copy_paths,
            ["go.work.sum".to_string(), "lib/go.sum".to_string()]
        );
    }
}
