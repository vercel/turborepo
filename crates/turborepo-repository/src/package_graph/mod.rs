...
pub struct WorkspacePackage<'a> {
    ...
    pub path: &'a AnchoredSystemPath,
}

impl WorkspacePackage<'_> {
    pub fn root() -> Self {
        Self {
            name: WorkspaceName::Root,
            path: AnchoredSystemPath::new(".").unwrap(),
        }
    }
}
                ...
                .map(|(name, _info)| match name {
                    WorkspaceName::Other(n) => {
                        ...
                        Some(WorkspacePackage {
                            name: w_name.clone(),
                            path: self.workspace_dir(&w_name).unwrap(),
                        })
                    }
        ...
        Ok(changed.unwrap_or_else(|| {
            self.workspaces
                ...
                .map(|name| WorkspacePackage {
                    name: name.clone(),
                    path: self.workspace_dir(name).unwrap(),
                })
                .collect()
        }))
    }
