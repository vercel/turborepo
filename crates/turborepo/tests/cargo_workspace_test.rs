...
//! pnpm/npm + Cargo fixture driven through the real turbo binary, covering
//! discovery, execution, caching, invalidation, output restoration, and the
//! two opt-in surfaces (`futureFlags.cargoWorkspaces` and
//! `TURBO_EXPERIMENTAL_CARGO`).
//!
...
/// The fixture's turbo.json opts in via `futureFlags.cargoWorkspaces`; no
...
fn test_filter_hint_when_cargo_disabled() {
    ...
    assert!(
        stderr.contains("cargoWorkspaces"),
...
/// The synthetic `cargo` package is not a pruneable target.
#[test]
fn test_prune_cargo_workspace_package_rejected() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("synthetic Cargo workspace package"),
        "expected guard message: {stderr}"
    );
}

/// The environment variable is the other opt-in surface: without the
/// future flag in turbo.json, `TURBO_EXPERIMENTAL_CARGO=1` produces the
/// same Cargo-aware graph. (Hash equality across the two surfaces is
/// asserted with an identical turbo.json in unit tests; here the files
/// necessarily differ, which changes the global hash.)
#[test]
fn test_env_var_opt_in_builds_cargo_graph() {
    let tempdir = tempfile::tempdir().unwrap();
    setup_cargo_monorepo(tempdir.path());
    ...
    // Without either surface, the crate is invisible.
    let output = run_turbo(tempdir.path(), &["build", "--filter=app"]);
    assert!(!output.status.success());

    // With the env var, the crate task exists and carries its command.
    ...
    assert_eq!(command, "cargo build --package=app");
}
