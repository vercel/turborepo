    ...
    match result {
        ...
        Err(err) => {
            ImageProcessingIssue {
                path: ident.path(),
                message: format!("{}", PrettyPrintError(&err)),
                issue_severity: None,
                title: None,
            }
            .cell()
...
fn load_image_internal(
    ...
    if matches!(format, Some(ImageFormat::Avif)) {
        ImageProcessingIssue {
            path: ident.path(),
            message: "This version of Turbopack does not support AVIF images, will emit without \
    ...
    if matches!(format, Some(ImageFormat::WebP)) {
        ImageProcessingIssue {
            path: ident.path(),
            message: "This version of Turbopack does not support WEBP images, will emit without \
                      optimization or encoding"
                .to_string(),
