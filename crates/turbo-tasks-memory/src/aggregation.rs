...
use turbo_tasks::{event::Event, RawVc, TaskId, TraitTypeId, TurboTasks, TurboTasksApi};

use crate::{aggregation_tree::AggregationContext, MemoryBackend};
...
struct CollectiblesInfo {
    collectibles: AutoMap<RawVc, isize>,
...
impl Default for Aggregated {
    fn default() -> Self {
        Self {
            unfinished: 0,
            unfinished_event: Event::new(),
            dirty_tasks: AutoMap::new(),
...
pub struct TaskAggregationContext<'a> {
    pub turbo_tasks: &'a dyn TurboTasksApi<MemoryBackend>,
...
impl<'a> TaskAggregationContext<'a> {
    pub fn new(
        turbo_tasks: &'a dyn TurboTasksApi<MemoryBackend>,
        backend: &'a MemoryBackend,
    ) -> Self {
        Self {
            turbo_tasks,
            backend,
            tasks_to_schedule: HashSet::new(),
        }
    }

    pub fn take_scheduled(&mut self, task: TaskId) -> bool {
        let mut task_to_schedule = self.tasks_to_schedule.get_mut();
        task_to_schedule
            .as_mut()
            .map(|t| t.remove(&task))
            .unwrap_or(false)
...
impl<'a> AggregationContext for TaskAggregationContext<'a> {
    type ItemLock = TaskMetaStateWriteGuard<'static>;
    ...
    fn item(&self, reference: TaskId) -> Self::ItemLock {
        todo!()
    }
    ...
    fn info_to_add_change(&self, info: &Aggregated) -> Option<Self::ItemChange> {
        ...
        for task in info.dirty_tasks.iter() {
...
fn update_count_entry<K, H>(entry: Entry<'_, K, i32, H>, update: i32) -> i32 {
    match entry {
        Entry::Occupied(mut e) => {
            let value = e.get_mut();
            *value += update;
