...
pub struct FutureFlags {
    ...
    /// `test`, `check`, `clippy`/`lint`, `bench`, and `doc`/`docs` tasks
    /// execute per crate for both entrypoints and libraries, making them
    /// selectable with `--filter`. The synthetic workspace package exposes
    /// explicit `test:workspace`, `check:workspace`, and corresponding
    /// aggregate tasks.
    ///
    /// Entrypoints implicitly register `build`; crates with one binary also
    /// register `run` and `dev`. All crates implicitly register the
    /// verification tasks, and the workspace package registers their
    /// `:workspace` forms. Normal task definitions configure or override
    /// these defaults, and package configuration can exclude them with
    /// `extends: false`.
    ///
