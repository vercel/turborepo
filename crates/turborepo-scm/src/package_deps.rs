...
mod tests {
    ...
    #[test]
    fn test_default_inputs_exclude_untracked_toolchain_caches() -> Result<(), Error> {
        let (_repo_root_tmp, repo_root) = tmp_dir();
        let package_path = AnchoredSystemPathBuf::from_raw("packages/app")?;
        let package_dir = repo_root.resolve(&package_path);
        package_dir.create_dir_all()?;
        package_dir
            .join_component("package.json")
            .create_with_contents(r#"{"scripts":{"build":"echo plain script"}}"#)?;
        package_dir
            .join_component("source.txt")
            .create_with_contents("source")?;
        // A user can deliberately track a file under one of these conventional
        // cache directory names. The default exclusion must not override SCM.
        package_dir
            .join_components(&["target", "tracked.txt"])
            .ensure_dir()?;
        package_dir
            .join_components(&["target", "tracked.txt"])
            .create_with_contents("tracked")?;
        ...
        for path in [
            ...
            let file = package_dir.join_unix_path(turbopath::RelativeUnixPath::new(path)?);
            file.ensure_dir()?;
            file.create_with_contents("generated")?;
        }

        let scm = SCM::new(&repo_root);
        let repo_index = scm.build_repo_index_eager().expect("git repo index");
        for index in [None, Some(&repo_index)] {
            let hashes = scm.get_package_file_hashes::<&str>(
                &repo_root,
                &package_path,
                &[],
                false,
            ...
            assert!(hashes.contains_key(&RelativeUnixPathBuf::new("source.txt")?));
            assert!(hashes.contains_key(&RelativeUnixPathBuf::new("target/tracked.txt")?));
            for generated in [
                "target/debug/generated",
                ".venv/lib/site.py",
                "src/__pycache__/module.pyc",
                ...
                assert!(
                    !hashes.contains_key(&RelativeUnixPathBuf::new(generated)?),
                    "default inputs unexpectedly hashed {generated}: {hashes:?}"
                );
            }
        }

        // Explicit inputs are filesystem-based and must remain able to opt the
        // conventional cache paths back into a task hash.
        let explicit = scm.get_package_file_hashes(
            &repo_root,
            &package_path,
        ...
        for path in [
            ...
            assert!(
                explicit.contains_key(&RelativeUnixPathBuf::new(path)?),
                "explicit input did not hash {path}: {explicit:?}"
            );
        }
    ...
    #[test]
    fn test_manual_default_inputs_exclude_toolchain_caches_but_explicit_inputs_include_them()
    -> Result<(), Error> {
        let (_tmp, root) = tmp_dir();
        let package_path = AnchoredSystemPathBuf::from_raw("app")?;
        let package_dir = root.resolve(&package_path);
        package_dir.create_dir_all()?;
        package_dir
            .join_component("source.py")
            .create_with_contents("print('source')")?;
        for path in [
            "target/debug/generated",
            ".venv/lib/site.py",
            "src/__pycache__/module.pyc",
        ...
        let defaults = get_package_file_hashes_without_git::<&str>(
            &root,
            &package_path,
            &[],
            false,
        ...
        assert!(defaults.contains_key(&RelativeUnixPathBuf::new("source.py")?));
        assert_eq!(defaults.len(), 1, "unexpected default inputs: {defaults:?}");

        let explicit = get_package_file_hashes_without_git(
            &root,
            &package_path,
            &["target/**", ".venv/**", "**/__pycache__/**"],
            false,
        ...
        for path in [
            ...
            assert!(explicit.contains_key(&RelativeUnixPathBuf::new(path)?));
        }
