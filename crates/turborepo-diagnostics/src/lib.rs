...
//!
//! This crate provides the diagnostic framework for running health checks
//! and troubleshooting functionality in turbo. It includes:
//!
//! - `DiagnosticMessage` - Messages sent from diagnostics to the UI
//! - `DiagnosticChannel` - Communication channel for diagnostic messages
//! - `Diagnostic` trait - Interface for implementing diagnostics
//! - Built-in diagnostics: Git FS Monitor, Daemon, LSP, Update
//!
...
use turborepo_daemon::{DaemonConnector, Paths as DaemonPaths};
...
impl DiagnosticChannel {
    ...
    pub async fn started(&self, message: String) {
        self.send(DiagnosticMessage::Started(message)).await
    }

    pub async fn log_line(&self, message: String) {
        self.send(DiagnosticMessage::LogLine(message)).await
    }

    /// prompts the user with a message and options
    pub async fn request(&self, message: String, options: Vec<String>) -> Option<Receiver<String>> {
    ...
    /// when it is done
    pub async fn suspend(&self) -> Option<(Receiver<()>, Sender<()>)> {
    ...
    pub async fn done(self, message: String) {
    ...
    pub async fn not_applicable(self, message: String) {
...
/// Diagnostic for checking and updating Turborepo to the latest version.
...
impl UpdateDiagnostic {
    pub fn new(repo_root: AbsoluteSystemPathBuf, current_version: &'static str) -> Self {
        Self {
            repo_root,
            current_version,
        }
