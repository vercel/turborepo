...
pub fn generate_register() {
    ...
    for (filename, entry) in entries {
        ...
        for ((mod_path, ident), ((global_name, trait_idents), _)) in values {
            writeln!(
                values_code,
...
struct RegisterContext<'a> {
    ...
    values: &'a mut HashMap<ValueKey, (ValueEntry, PathBuf)>,
}
...
impl<'a> RegisterContext<'a> {
    ...
    fn add_value(&mut self, ident: &Ident) {
        ...
        let old_value = self.values.insert(key, (value, self.file_path.clone()));
        if let Some((_, path)) = old_value {
            panic!(
                "{} is declared more than once in {:?} and {:?}",
                ident, self.file_path, path
            );
        }
    ...
    fn add_value_trait(&mut self, ident: &Ident, trait_ident: &Ident) {
        ...
        match entry {
            Some(entry) => entry.0 .1.push(trait_ident.clone()),
            None => {
                panic!(
                    "failed to add value trait {} to {} in {}. Did you try to implement a trait \
                     on a Vc instead of its value?",
                    trait_ident,
                    ident,
