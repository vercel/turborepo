...
pub async fn get_browser_runtime_code(
    ...
    let runtime_base_code =
        embed_static_code(asset_context, "browser/runtime/base/runtime-base.ts".into());
    ...
    let runtime_backend_code = embed_static_code(
        asset_context,
        match chunk_loading {
            ChunkLoading::Edge => "browser/runtime/edge/runtime-backend-edge.ts".into(),
            // This case should never be hit.
            ChunkLoading::NodeJs => {
                panic!("Node.js runtime is not supported in the browser runtime!")
            }
            ChunkLoading::Dom => "browser/runtime/dom/runtime-backend-dom.ts".into(),
        },
    );
