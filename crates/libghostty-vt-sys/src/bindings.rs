...
#[doc = " A 256-bit mask of palette indices.\n\n Index i is set iff `(bits[i >> 6] >> (i & 63)) & \
         ...
         GHOSTTY_COLOR_PALETTE_MASK_SET(&mask, 20);\n if (GHOSTTY_COLOR_PALETTE_MASK_IS_SET(&mask, \
...
unsafe extern "C" {
    #[doc = " Parse an X11 color name.\n\n The color name is resolved from Ghostty's embedded \
...
unsafe extern "C" {
    #[doc = " Parse a flexible Ghostty color value.\n\n Accepts Ghostty's terminal color syntax: \
             X11 color names, hex colors\n in 3-, 6-, 9-, or 12-digit form (the leading # is \
...
unsafe extern "C" {
    #[doc = " Parse a Ghostty palette entry.\n\n Accepts Ghostty palette config syntax: N=COLOR. N \
             is a palette index\n from 0 to 255 in decimal or in 0x, 0o, or 0b-prefixed form. \
...
unsafe extern "C" {
    #[doc = " Get Ghostty's built-in default 256-color palette.\n\n Writes exactly 256 entries: \
             Ghostty's base16 defaults, the xterm\n 6x6x6 color cube, and the grayscale ramp.\n\n \
             GhosttyColorRgb values\n"]
    pub fn ghostty_color_palette_default(out: *mut ColorRgb);
}
unsafe extern "C" {
    #[doc = " Generate a 256-color palette from base colors.\n\n The base palette supplies indices \
             0-15, which are always preserved.\n If @p base is NULL, Ghostty's default palette is \
             used. If @p skip is\n NULL, no extra indices are skipped. Set bits in @p skip \
             preserve those\n indices from @p base. The 216-color cube at indices 16-231 is \
             ...
             @p harmonious controls whether the generated palette\n keeps the \
             background-to-foreground orientation. When false, Ghostty\n swaps the light \
             background and dark foreground so the cube and ramp run\n dark-to-light. The output \
             palette may be the same pointer as @p base.\n\n values, or NULL to use Ghostty's \
             default palette\n an empty mask\n orientation\n GhosttyColorRgb values\n"]
...
unsafe extern "C" {
    #[doc = " Calculate perceived luminance for an RGB color.\n\n Returns a normalized value from \
             0.0 for black to 1.0 for white.\n Ghostty treats a background color as light when \
...
unsafe extern "C" {
    #[doc = " Calculate the WCAG contrast ratio between two RGB colors.\n\n The contrast ratio is \
             symmetric and ranges from 1.0 for identical\n colors to 21.0 for black and white.\n\n"]
    pub fn ghostty_color_contrast(a: *const ColorRgb, b: *const ColorRgb) -> f64;
}
unsafe extern "C" {
    #[doc = " Get Ghostty's X11 color name table.\n\n The returned pointer references static \
             memory valid for the program\n lifetime and is never NULL. Entries are in rgb.txt \
             order and are\n terminated by an entry with name == NULL. Aliases are separate \
             ...
             case-insensitively.\n\n for (const GhosttyColorX11Entry* e = \
             ghostty_color_x11_names();\n      e->name != NULL;\n      e++) {\n   // e->name and \
             e->color are valid here.\n }\n\n"]
    pub fn ghostty_color_x11_names() -> *const ColorX11Entry;
}
unsafe extern "C" {
    #[doc = " Get the number of X11 color name entries.\n\n The returned count excludes the NULL \
             terminator and is provided so\n bindings can preallocate storage before reading \
             ghostty_color_x11_names().\n\n"]
...
unsafe extern "C" {
    #[doc = " Encode a color scheme report into an escape sequence.\n\n Encodes a color scheme \
             report into the provided buffer. Dark color schemes\n emit ESC [ ? 997 ; 1 n, and \
             light color schemes emit ESC [ ? 997 ; 2 n.\n The encoded bytes are identical to the \
             terminal's internal CSI ? 996 n\n query response.\n\n Hosts should gate unsolicited \
             sends on GHOSTTY_MODE_COLOR_SCHEME_REPORT\n (mode 2031) being set, which can be \
             checked via the mode getters.\n\n If the buffer is too small, the function returns \
             GHOSTTY_OUT_OF_SPACE\n and writes the required buffer size to @p out_written. The \
...
pub mod KittyGraphicsData {
    ...
    #[doc = " Generation stamp of the last content mutation to this storage:\n any image \
             transmit/replace, placement add, or delete. Zero means\n the storage has never been \
             mutated (and is therefore empty).\n\n If the generation is unchanged since a previous \
             query, the set of\n placements and all image data are identical, so placement \
             iteration\n and image staleness checks can be skipped entirely. Note that\n placement \
             *geometry* may still have changed (scrolling and resizing\n move placements without \
             changing the storage contents), so rendering\n geometry such as \
             ghostty_kitty_graphics_placement_render_info()\n must still be recomputed for frames \
             ...
             screen switches (main vs. alternate screen have\n independent storages) or terminal \
             resets. It is therefore safe to\n key caches on this value alone.\n\n Output type: \
             uint64_t *"]
...
pub mod KittyGraphicsImageData {
    ...
    #[doc = " Generation stamp assigned when this image was added to (or\n replaced in) the \
             ...
             have changed even when the\n dimensions, format, and data length are identical (e.g. \
...
pub mod TerminalScrollViewportTag {
    ...
    #[doc = " Scroll to an absolute row offset from the top of the scrollable\n area. Row 0 is the \
             top of the scrollback and the requested row\n becomes the first visible row of the \
             viewport. The value is\n clamped so the viewport never scrolls beyond the top of \
             the\n active area. If the terminal has no scrollback (e.g. the\n alternate screen is \
             active), the viewport always remains on the\n active area.\n\n This is the same row \
             space as the offset field of\n GhosttyTerminalScrollbar, so a scrollbar position \
...
#[doc = " One MIME representation in a clipboard write.\n\n Both strings are borrowed and valid \
         only for the duration of the callback.\n The data is binary-safe and has already been \
         decoded from any protocol-level\n encoding. A zero-length data string is an explicit \
         empty representation; it\n does not clear the clipboard.\n\n This struct has a frozen \
         layout and will not gain fields in future versions.\n"]
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
pub mod TerminalData {
    ...
    #[doc = " Scrollbar state for the terminal viewport.\n\n This is amortized O(1): the total is \
             maintained incrementally as\n the terminal is modified and the viewport offset is \
             ...
             the offset, after which it is cached again.\n\n There is intentionally no change \
             notification for scroll state.\n Callers building scrollbars should poll this once \
             per frame or\n per write batch and diff the result to detect changes; this is\n what \
             Ghostty's own renderer does.\n\n Output type: GhosttyTerminalScrollbar *"]
...
unsafe extern "C" {
    #[doc = " Scroll the terminal viewport.\n\n Scrolls the terminal's viewport according to the \
             ...
             for down). When using GHOSTTY_SCROLL_VIEWPORT_ROW,\n set the row field to the \
             absolute row offset from the top of the\n scrollable area (the same row space as the \
             offset field of\n GhosttyTerminalScrollbar). For other behaviors, the value is \
             ignored.\n\n"]
    pub fn ghostty_terminal_scroll_viewport(terminal: Terminal, behavior: TerminalScrollViewport);
}
unsafe extern "C" {
    #[doc = " Return the current compression activity token.\n\n The token is opaque and only \
             equality comparisons are meaningful. An\n embedding application should cache it and \
             restart its compression idle\n delay whenever the value changes. The value may wrap \
             and changes in either\n direction have the same meaning.\n\n This function only \
             observes terminal state. It does not perform or schedule\n compression.\n\n         \
             is NULL\n"]
...
unsafe extern "C" {
    #[doc = " Compress eligible terminal scrollback.\n\n Incremental mode performs bounded work \
             suitable for an idle callback. A\n pending result means the application should invoke \
             another step while the\n terminal remains idle. A complete result means no \
             continuation is needed\n until ghostty_terminal_compression_activity() changes. Full \
             ...
             page was compressed: pages may be unprofitable or encounter an\n allocation or \
             reclamation failure. Compression changes only the terminal's\n storage representation \
             and never its logical contents or scrollback limit.\n Accessing compressed history \
             restores it transparently.\n\n This function is not thread-safe with other operations \
             on the same\n terminal. The caller must serialize it with writes, rendering, \
...
unsafe extern "C" {
    #[doc = " Update a render state instance from a terminal.\n\n This consumes terminal/screen \
             ...
             convenience function that performs a full update in one call,\n equivalent to \
             ghostty_render_state_begin_update immediately followed by\n \
             ghostty_render_state_end_update. Callers that hold a lock over the\n terminal state \
             should prefer calling the two phases directly so that the\n lock is only held for the \
             begin phase.\n\n `terminal` is NULL, GHOSTTY_OUT_OF_MEMORY if updating the state \
...
unsafe extern "C" {
    #[doc = " Begin an update of a render state instance from a terminal.\n\n Every begin must be \
             completed with a ghostty_render_state_end_update call\n before the render state is \
             ...
             and then call ghostty_render_state_end_update after releasing\n it. The end phase \
             exclusively reads and writes memory owned by the render\n state, so it is safe to \
             call while the terminal is being modified.\n\n Work that doesn't require terminal \
             access may be deferred to the end phase\n to keep this call (and therefore lock hold \
             time) as short as possible.\n Callers must treat the render state as incomplete \
...
unsafe extern "C" {
    #[doc = " Complete a prior ghostty_render_state_begin_update call by performing any\n deferred \
             ...
             required). Calling this without a prior begin is a safe no-op.\n\n NULL\n"]
...
unsafe extern "C" {
    #[doc = " Measures the terminal display width of the first grapheme cluster in a\n sequence of \
             Unicode codepoints.\n\n This uses the exact same grapheme segmentation and cluster \
             ...
             overlays) that exactly matches what the terminal will do when\n the text is actually \
             written to it. Unlike\n ghostty_unicode_codepoint_width(), this accounts for \
             cluster-level\n rules: emoji variation selectors, ZWJ sequences, combining marks, \
             and\n skin tone modifiers.\n\n Reads codepoints from cps until the terminal would \
             ...
             returns the number of codepoints consumed. Returns 0 if\n and only if len is 0; \
             otherwise consumes at least one codepoint. Measure\n a whole string by calling in a \
             loop:\n\n size_t total = 0;\n for (size_t i = 0; i < len;) {\n   uint8_t width;\n   i \
             += ghostty_unicode_grapheme_width(cps + i, len - i, &width);\n   total += width;\n \
             }\n\n This is not a streaming API. The provided sequence must contain a\n complete \
             first grapheme cluster, or the logical end of the string. If\n input arrives in \
             chunks, keep buffering while this function consumes all\n available codepoints \
             (return value == len) and the stream may still\n continue; a later codepoint could \
             still extend the cluster and change\n its width.\n\n Width semantics, matching the \
             terminal with mode 2027 enabled:\n - The cluster starts at the width of its first \
             codepoint, as returned by\n   ghostty_unicode_codepoint_width().\n - VS16 (U+FE0F) \
             forces the cluster wide (2) and VS15 (U+FE0E) forces it\n   narrow (1), but only when \
             the immediately preceding codepoint in the\n   cluster is a valid emoji variation \
             sequence base (per Unicode\n   emoji-variation-sequences.txt). Invalid variation \
             selectors are\n   ignored entirely.\n - Any other continuation codepoint that \
             contributes to grapheme width\n   forces the cluster wide (2). Note this means \
             cluster width is NOT the\n   maximum of per-codepoint widths: some continuation marks \
             have narrow\n   codepoint width yet still widen the cluster.\n\n Mode dependence: \
             this models mode 2027 (grapheme clustering) enabled,\n which is Ghostty's recommended \
             configuration. When mode 2027 is\n disabled, clusters never combine and variation \
             ...
             cluster boundaries. This function is total; it never fails.\n - Control characters \
             (C0/C1, CR, LF) are never printed through the\n   terminal's text path; passing them \
             here returns an unspecified (but\n   stable and bounded) result.\n - A cluster whose \
             first codepoint is zero-width (e.g. a lone combining\n   mark) is malformed at a cell \
             start; the terminal may attach it to\n   earlier screen content. This function \
             reports the fold result for the\n   sequence in isolation (typically 0).\n\n This \
             function is pure, allocates nothing, and is thread-safe.\n"]
