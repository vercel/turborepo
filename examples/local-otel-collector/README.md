...
This example shows how to build the `turbo` binary and send metrics to a local collector running via **Docker Compose**. It's intended as a lightweight, local integration harness for the OTEL exporter.
...
This produces the `turbo` binary at: `./target/debug/turbo`.
...
```bash
cd examples/local-otel-collector
docker compose up -d
```

This starts:

- **`otel-collector`** (OTLP gRPC receiver + debug + Prometheus exporter)
...
The `turbo` binary remains available at `./target/debug/turbo`. You can continue using it with the same environment variables to send metrics to this collector or to another OTLP-compatible backend.
