...
- Release PRs are created by the `turbobot` account using the `TURBOBOT` secret, approved by `github-actions`, and squash-merged automatically after required checks pass. Turborepo release PR titles use `chore: Release Turborepo <version>`; the author and title are part of the trusted release-PR validation contract.
