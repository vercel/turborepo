...
Run build with package task in non-root turbo.json
  ...
  $ sed  's/\[\([^]]*\)\]/\(\1)/g' < error.txt
    ...
    `-> unnecessary_package_task_syntax (https://turborepo.dev/messages/
...
Run in single package mode even though we have a task with package syntax
  ...
  [1]
  $ sed  's/\[\([^]]*\)\]/\(\1)/g' < error.txt
  package_task_in_single_package_mode (https://turborepo.dev/messages/package-task-in-single-package-mode)
