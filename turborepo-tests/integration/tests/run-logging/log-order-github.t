...
because otherwise prysk interprets them as multiline commands
  ...
  ::group::my-app:build
  cache bypass, force executing 0dc3529515348bdf
  ...
  ::endgroup::
  ::group::util:build
  cache bypass, force executing c57488561460f6ed
...
# Build as if we are in Github Actions with a task log prefix.
  ...
  \xe2\x80\xa2 Remote caching disabled (esc)
  ::group::util:build
  util:build: cache bypass, force executing c57488561460f6ed
  util:build: 
  util:build: > build
...
Verify that errors are grouped properly
  ...
  \xe2\x80\xa2 Remote caching disabled (esc)
  ::group::util:fail
  cache miss, executing 70a63687ca99269f
