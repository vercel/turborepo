...
Hash changes, because we're using a new mode
  $ WITH_FLAG=$(${TURBO} build -vv --experimental-env-mode=strict 2>&1 | "$TESTDIR/../_helpers/get-global-hash.sh")
...
Hash does not change, because the mode is the same and we haven't added any new pass through vars
  ...
  $ WITH_EMPTY_GLOBAL=$(${TURBO} build -vv --experimental-env-mode=strict 2>&1 | "$TESTDIR/../_helpers/get-global-hash.sh")
...
Hash changes, because we have a new pass through value
  ...
  $ WITH_GLOBAL=$(${TURBO} build -vv --experimental-env-mode=strict 2>&1 | "$TESTDIR/../_helpers/get-global-hash.sh")
