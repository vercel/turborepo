...
It finds repo root and uses correct version
  ...
  [-0-9:.TWZ+]+ \[DEBUG] turborepo_lib::shim: Local turbo path: .*(\/|\\)hard_mode.t(\/|\\)subdir(\/|\\)node_modules(\/|\\)turbo-(darwin|linux|windows)-(64|arm64)(\/|\\)bin(\/|\\)(turbo|turbo\.exe) (re)
  [-0-9:.TWZ+]+ \[DEBUG] turborepo_lib::shim: Local turbo version: 1.8.0 (re)
  [-0-9:.TWZ+]+ \[DEBUG] turborepo_lib::shim: Running local turbo binary in .*(\/|\\)hard_mode.t(\/|\\)subdir(\/|\\)node_modules(\/|\\)turbo-(darwin|linux|windows)-(64|arm64)(\/|\\)bin(\/|\\)(turbo|turbo\.exe) (re)
